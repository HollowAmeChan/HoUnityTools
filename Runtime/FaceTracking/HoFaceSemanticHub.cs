...
namespace Hollow.HoUnityTools.FaceTracking
{
    ...
    ///   · **别人也能写**：作者自己挂在角色上的组件、别的 mod 的脚本，`ClaimSlot` + `SetFloat` 就行
    ///     （<see cref="HoFaceSemanticConnector.SetFloat"/> 就是给这个用的）。所以"Hub 里能有什么"是
    ///     **产品边界**（我们提供中间层这一条），不是**机制边界**。
    ///   · ⚠️ **唯一写不进去的是动画 clip**：曲线写 `values.<i>` 那条路 2026-09-26 删了
    ///     （绑定是静态字符串 ⇒ 作者必须先知道下标，细节见 `docs/FACE_TRACKING_DYNAMIC_PARAMETERS.md` §5.1）。
    ///
    /// ⚠️ **曾经还有一个"跑在控制器里"的写者**：控制器状态上的状态机行为
    /// （`HoFaceSemanticWriterBehaviour`，2026-09-26 上午加、下午删）。删它的理由：它的活中间层
    /// 本来就能干，留着就是两份真相 + 一个只在 bundle 里跑、编辑器里看不见的写者；
