...
namespace Hollow.HoUnityTools.Editor.AnimationTools
{
    ...
    internal sealed class HoAnimationToolsWindow : EditorWindow
    {
        ...
        private void DrawMeshList()
        {
            if (meshes.Count == 0)
            {
                HoConstraintEditorControls.Caption(KeyCount() == 0
                    ? "还没有网格 —— 先填根物体，再点「按根物体填充」。"
                    : "列表是空的。");
            }
            else
            {
                meshScroll = EditorGUILayout.BeginScrollView(meshScroll, GUILayout.Height(ListHeight()));
        ...
        private void DrawResultList()
        {
            ...
            resultScroll = EditorGUILayout.BeginScrollView(resultScroll, GUILayout.Height(110.0f));
            foreach (string name in reportNames) HoConstraintEditorControls.Caption(name);
            EditorGUILayout.EndScrollView();
        }

        /// <summary>网格列表的有界高度：窗口越高给得越多，但始终留得下别的行。</summary>
        private float ListHeight() => Mathf.Clamp(position.height - 360.0f, 120.0f, 420.0f);
