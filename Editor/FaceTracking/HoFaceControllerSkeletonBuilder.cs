...
namespace Hollow.HoUnityTools.Editor.FaceTracking
{
    ...
    public static class HoFaceControllerSkeletonBuilder
    {
        ...
        /// <summary>
        /// **`Mouth/Roll` 的两个阈值：0.02 / 0.12**（2026-09-27 用户实测定）。
        /// `0.02` = 死区出口（轴在这一行里已经把 ≤0.02 归零）；`0.12` = **猫嘴满档**。
        /// ⚠️ 满档阈值**故意低于实测的 0.18**（只卷嘴的读数）：给"读数被压"留余量 ——
        /// 张嘴时设备会把这根轴压低，贴着峰值设阈值会让猫嘴在张嘴时只淡入一部分。
        /// 轴的表达式里还有**下颌增益**（`1 + 4*jawOpen²`）把读数乘回来，两条一起给余量；
        /// 实测咬唇最强 0.45 也停在满档（用户定「不需要区分两段卷嘴」）。
