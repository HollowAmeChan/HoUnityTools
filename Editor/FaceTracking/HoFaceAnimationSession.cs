...
namespace Hollow.HoUnityTools.Editor.FaceTracking
{
    ...
    public sealed class HoFaceAnimationSession : IDisposable
    {
        ...
        /// <summary>
        /// 面捕凝视与 HoLookAt 眼球不能同时朝一个方向使劲：两者都写眼睛，叠起来是双倍偏转。
        /// 这是配置冲突，直接报出来让用户选一个，而不是悄悄各写一半。
        /// </summary>
        private void RejectGazeConflict()
        {
            bool gaze = false;
            for (int i = 0; i < selected.Length; i++)
            ...
            foreach (var lookAt in animator.GetComponentsInChildren<HoLookAtConstraint>(true))
                if (lookAt.EyeOutputEnabled)
                    throw new InvalidOperationException("凝视输出与 HoLookAt 冲突。请关闭面捕凝视分组，或将 LookAt 的眼睛外部权重设为 0。");
        }
