...
namespace Hollow.HoUnityTools.Editor.FaceTracking
{
    ...
    ///
    /// **当前状态：未接线。** §19.5 指定的落点是这里（"它的两个本质部件改成生成器的基础件"），
    /// 但生成器后来自己手搓了树（`LidPoses` 表 + 手写区域子树），所以**目前只有用例在用它**。
    /// 两件事没做：① 生成器改用这里造树；② "把成对反向通道合成一根 −1~1 轴 + 三姿势 1D 树"
    /// （现在那批通道全是直通叶子，双向轴根本没有树）。要么接线，要么连它一起删 —— 别让它悬着。
    ///
