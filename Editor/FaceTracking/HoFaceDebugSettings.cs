...
namespace Hollow.HoUnityTools.Editor.FaceTracking
{
    ...
    public sealed class HoFaceDebugSettings
    {
        ...
        /// <summary>
        /// 进入播放后自动开会话（**不会**自动连手机）。**默认开**（2026-09-27 用户定）：
        /// 面捕调试的常态就是"按 Play 就开始跑"，每次进来手按一遍没有信息量。
        /// 面板入口 = 「对象」段最底下那行布尔开关里的「**自动驱动**」：
        ...
        /// 现在两边口径一致：**空 = 空表**。
        /// ⚠️ 而且**内置默认表本身已经删掉了**（2026-09-26，`HoFaceMiddlewareDefaults` 整个类）：
        /// 它当时的两个角色（`WriteDefaults()` 的导出文本、验证用例的夹具）一个没了对象、
        /// 一个搬进用例里自己搭 —— **仓库里现在不存在任何一份默认配置**。
        /// </summary>
