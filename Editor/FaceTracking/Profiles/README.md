...
| 文件 | 是什么 |
|---|---|
| `ho-iPhoneVTS.hoface.json` | **正式中间层**（这一层现在只有它）：67 条输入行 + **130 行输出** = **90 条出口**（原始 ARKit 52 + 官方 VTS 20 + VB 自造 5 + 姿态向量 12 + `FaceFound`，2026-09-27 之前只有这 90 行）+ **40 行控制器轴 `Ho/Drive/*`**（轴 / 区域门 / `MouthCore` 切片权重，喂新控制器；逐行见 `docs/PARAMETER_HO.md` §3.7）。公式机械取自本机 VBridger，见下 |
...
出口是**两份并行 + 一套额外**，共 **90 行**；**2026-09-27 起再加 40 行控制器轴 `Ho/Drive/*`**
（轴 + 区域门 + `MouthCore` 切片权重，喂 [新控制器](../Controllers/README.md)——逐行表达式与曲线见
`docs/PARAMETER_HO.md` §3.7）。**两组是并列的两份"面"**：G1–G3 是给下游的**出口名**，
`Ho/Drive/*` 是给控制器的**口径**；轴行是**新增**而不是把出口名改掉（出口名是下游契约）。
...
它是"改了生成器之后忘了重新生成"的唯一守门人。当前：**154 passed / 0 failed**。
