...
| 文件 | 是什么 |
|---|---|
| `ho-iPhoneVTS.hoface.json` | **正式中间层**（这一层现在只有它）：67 条输入行 + **127 行输出** = **90 条出口**（原始 ARKit 52 + 官方 VTS 20 + VB 自造 5 + 姿态向量 12 + `FaceFound`，2026-09-27 之前只有这 90 行）+ **37 行控制器轴 `Ho/Drive/*`**（轴 / 区域门 / `MouthCore` 切片权重，喂新控制器；逐行见 `docs/PARAMETER_HO.md` §3.7）。公式机械取自本机 VBridger，见下 |
...
出口是**两份并行 + 一套额外**，共 **90 行**；**2026-09-27 起再加 37 行控制器轴 `Ho/Drive/*`**
