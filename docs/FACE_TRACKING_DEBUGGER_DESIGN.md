...
日期：2026-09-22。状态：**P0–P2 已实现并在独立 Unity 工程中跑通自动化验收；P3 的挂钩已就位但未做真机与人类眼球骨骼验收；真机（手机）联调未做**。先读 [混合树与 ARKit 模板使用](FACE_TRACKING_TEMPLATE_GUIDE.md) 可了解背景。
...
### 5.1 角色组件 `HoFaceTrackingDebugger`（拟定）

挂在角色根或工具子物体上，显式选择 **Target Animator**。绑定根默认就是该 Animator 的 Transform；不允许随便选一个不一致的“根”而隐式更改 Unity 的绑定规则。

```text
Ho 面捕调试
  角色 Animator        [Character]
  输入连接             [iFacialMocap 手机]
  面部 Controller      [Face_ARKit.controller]
  参数 Profile         [iFacialMocap → ARKit]
  [生成 ARKit 控制器] [检查绑定] [定位动画] [开始驱动 / 停止]
  ...
  输入参数
    参数 | 原值 | 路由后 | Controller 实值 | 来源 | 时间 | 状态
    每行可选：实时 / 手动 / 保持 / 中性 / 交还

  绑定检查
    Body → Meshes/Face
    匹配的 Renderer、键数、缺失键、路径错误、冲突写入者

  动画检查
    当前状态、图层权重、选中树、最终形态键值
```

**不把 Mesh 列表作为 Animator 模式必填项。** 从 Clip 绑定和 Animator 根解析受影响对象。提供 Renderer 对象选择器，是为了生成路径重映射；选择对象后计算相对路径，展示 `Body → Meshes/Face`，然后应用于临时片段副本。对象必须位于绑定根下。

多 Mesh 需逐条映射；同名键可以属于不同 Renderer。若某模型多个网格都需要同一曲线，需明确选择扇出目标，不能默认扫描到谁就全部写。缺键不创建形变，不重命名模型，不修改包内源片段。

### 5.2 全局面板 `HoUnityTools/面捕调试`（拟定）

负责连接、设备与会话，不存角色的动画曲线绑定。首屏只显示：`iFacialMocap`、手机 IP、电脑 IP 提示、连接状态、活动角色、连接/断开按钮。UDP 49983 作为协议默认值，不要求普通用户理解双向端口；高级区保留实际绑定信息。手机目前手动填 IP，不宣称自动扫描发现设备。

高级区显示：本地网卡 IP、目标手机、实际来源端点、接收包/字段速率、最近收包时间、无效包数、未知参数、端口占用错误及映射 Profile。默认接受选定手机的来源地址；首次收包核对失败时直接显示差异，切换设备需显式操作。

连接状态至少有“已停止 / 等待手机响应 / 收到数据 / 无匹配参数 / 长时间未更新 / 暂停驱动 / 错误”。基础协议未给出明确的人脸有效标志时，不根据全零表情声称“未检测到人脸”。UDP Socket 已打开不叫“设备已连接”。界面刷新建议限于 10–20 Hz，动画按 Unity 帧率执行，避免每个 UDP 包都 Repaint。
...
| 凝视形态键 | 默认无面捕输出 | 如果 LookAt 改为形态键模式，交给 LookAt |
...
启动保存原 Controller、Animator 相关设置和会话实际会写的属性；临时图/片段不保存到源资产。停止时先停本来源写入并撤销拥有权，再恢复配置并让基础动画接回。只有没有接替者的属性才还原保存值；旧 Writer 的 RestoreWritten 不能在新拥有者已写完后再次覆盖。
...
一键生成器扫描角色实际存在的 ARKit 键，生成 `ARKit/<键名>` Float 参数，以及 0/100 两个姿势组成的 1D 子树。**每个形态键一个独立 Override 图层**，而不是把所有通道塞进一个 Direct 树 —— Direct 树的归一化会让"同时张嘴与微笑"互相削弱。凝视输出默认排除，眼睑保留；自定义额外键通过 Profile 配置。生成资产保存到用户项目的新目录，不覆盖既有控制器；运行预览所需的过滤副本仅保存在内存。
