...
负责连接、设备与会话，不存角色的动画曲线绑定。首屏只有四块：**手机连接**（手机 IPv4、电脑 IPv4、连接/断开）、**角色**（调试组件、开始/停止驱动）、**参数**（52 路的键名 / 模式 / 原值 / 横条 / 面捕输入）、**排查**。UDP 49983 作为协议默认值，不要求普通用户理解双向端口。手机目前手动填 IP，不宣称自动扫描发现设备。

**排版原则：平时安静，出事才响。** 面板常态只留三类东西 —— 状态胶囊、当前该点的按钮、参数读数。具体做法：

- **说明文字一律进 tooltip**，不占版面。以前「手机和电脑需能通过局域网互通，App 保持前台……」这类整段话是常驻的，现在挂在对应控件的 tooltip 上。
- **分区可折叠**（复用仓库既有的 `HoConstraintEditorControls.Section` + 卡片 + 行栅格，与眨眼/注视面板同一套设计令牌），摘要行在收起时也报关键数字。
- **「排查」默认收起**，但它会在**出现新问题时自己弹开一次**，之后再不和用户较劲（用户收起就是收起）。这样"正常"时它是安静的，"出事"时它主动出现。
- **`HelpBox` 只用于真问题**（端口占用、socket 异常、绑定缺失）。操作结果（例如防火墙授权成功/失败）用一行小字，不用框。
...
角色 Inspector 用同一套排版：标题行带状态胶囊（驱动中 / 播放中），输出分组与 52 路参数各自折叠，通道同样压成一路一行，模式的五个选项用带 tooltip 的下拉。
