...
| 凝视形态键 | **面捕**（默认开） | 与 LookAt 的眼球开关**二选一**；两边都开只提示不阻塞 |
...
凝视输出**默认也开**（LookAt 不是一定存在）；要不要把凝视让给 LookAt 由两边各自的开关决定，两边都开时给提示但不阻塞。眼睑默认保留；自定义额外键通过 Profile 配置。生成资产保存到用户项目的新目录，不覆盖既有控制器；运行预览所需的过滤副本仅保存在内存。
...
| **后续 C：VRC 后端 + 曲线驱动参数桥** | **已明确推迟（短期不做）** | 见 [面捕的层间分工](FACE_TRACKING_PIPELINE_SPLIT.md) 第 14 节的**推迟项备忘**：触发条件、要做成什么样、证据出处都记在那里。不是放弃，是等触发条件 |
