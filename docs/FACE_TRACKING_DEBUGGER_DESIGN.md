...
> [面捕控制器结构](FACE_TRACKING_CONTROLLER_STRUCTURE.md)（控制器是作品，初始化只负责搬运）。
...
**驱动器列表是必填项（2026-09-23 改）**：早先设计成"从 Clip 绑定和 Animator 根自动解析受影响对象、只把 Renderer 选择器当路径重映射的来源"，理由是"少一个必填项"。改成搬运模型后**反过来**了：控制器是搬来的作品，它的曲线绑在作者自己的层级上（比如 `Source/Face`），**必须**由用户指定的网格列表来回答"这些动画写到谁身上"。于是路径重映射这一栏删掉了 —— 它做的事情被"初始化时重绑"取代，而且做得更彻底（一个键可以扇出到列表里所有有这个键的网格）。

**但"不默认扫全部、缺键不创建形变、不重命名模型、不改包内源片段"这几条不变**：扫描只在"按 Animator 填充"这个**显式动作**里做一次，扫描结果摆在列表上等你删改；源控制器与它引用的外部 `.anim` 一个字节都不动。
...
Runtime/FaceTracking/
  HoFaceTrackingDebugger.cs        角色组件：Animator、面部 Controller、源控制器、驱动对象列表、输出分组、输入通道
...
Editor/FaceTracking/
  ...
  HoFaceTrackingDebuggerEditor.cs  角色 Inspector：五个分区、52 路输入表、绑定检查、初始化（搬运）
  ...
  HoFaceAnimationAssets.cs         搬运（整份复制 + 重绑驱动对象）与预览编译（临时 OverrideController + 过滤后的临时 Clip）
  HoFaceAnimationSession.cs        影子求值台、输入门控、键拥有权、写回与停止恢复
Tests~/FaceTrackingValidation.cs   独立验证工程的批处理用例（93 条断言）
```
...
初始化**不再生成树**（2026-09-23 删除生成器）。它只做搬运：把**源控制器**整份复制到目标路径，
再把每条形态键曲线重绑到**驱动对象**列表里的网格上；源里引用的外部 `.anim` 先复制成本文件的
...
> 2. 然后改成"按模板生成一棵 Direct 树 + 眼睑 2D 树"（模板是代码里的数据表）。**这一步也删了**：那等于在代码里重新发明一遍混合树编辑器，还得把"轴的值怎么算"这种中间层的事写进控制器。控制器是**作品**，在混合树编辑器里编，自带片段与动画；代码只搬运它，并按"驱动对象里谁有这个键"重绑动画。
>
> 代价是诚实的：**我们不再保证控制器的形状**，面板因此改成**从资产读实况**（几层几个状态、写哪些键、哪些键模型上没有）。详见 [面捕控制器结构](FACE_TRACKING_CONTROLLER_STRUCTURE.md) §5。

凝视输出**默认也开**（LookAt 不是一定存在）；要不要把凝视让给 LookAt 由两边各自的开关决定。两边都开就是眼睛方向被写两遍、肉眼可见，**不做冲突检测** —— 检测要每帧扫一遍 LookAt 组件，为一个用户自己看得见的问题付这个代价不值得。眼睑默认保留；自定义额外键通过 Profile 配置。**运行预览所需的过滤副本（只含我们拥有的键）仅保存在内存**；落盘的只有初始化产出/覆盖的那一个控制器文件。

搬来的控制器可以带 Jerry 那份模板的形状（甚至它自带的风格化联动），只要它在运行期范围内：
...
| P1：手动动画 | **已通过** | 手动值驱动搬运过来的控制器得到 60/80/25/40；同时张嘴与微笑不被归一化削弱；`mouthClose` 与 `jawOpen` 独立；驱动对象列表把绑定重绑到真实 Renderer（含一个键扇出到两个网格）；凝视键排除后基础动画的 33 与身体变换的 2 都保留；停止后交还 |
...
已完成的验证：独立 Unity 6000.3 工程里用**本地回环 UDP** 替代手机，跑通了协议解析、接收器、控制器搬运（整份复制 / 外部片段复制 / 重绑驱动对象 / 覆盖不改 GUID）、绑定重映射、混合树求值、输入门控、键占据与交还、停止恢复，93 条断言全绿。
...
- **带 Behaviour / 同步图层 / 非形态键曲线的控制器**：搬运不拦，**开始驱动会被拒**；还没试过把这类控制器搬到影子台上跑。
- **VRCFT**：完全没有实施。
