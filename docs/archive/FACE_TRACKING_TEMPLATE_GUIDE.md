...
调查日期：2026-09-22。本文是使用说明与源码调查；端到端链路已在独立 Unity 工程里用本地回环 UDP 跑通自动化验收，**真机联调未做**。组件方案见 [面捕调试组件设计](../FACE_TRACKING_DESIGN.md)。
...
“完全掌握动画控制”应体现为：每个参数来自哪里、每个最终属性由谁控制、何时混合、停用后交给谁，都能看见。它不要求把网络接收、目标方向解算和全部程序逻辑硬塞进状态机 —— 当前实现也确实没这么做：网络、门控、方向解算都在组件里，控制器只负责"参数 → 姿势"。具体门控与执行方案见 [设计文档](../FACE_TRACKING_DESIGN.md) 第 7 节。
