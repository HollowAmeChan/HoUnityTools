...
| 仓库 | 是什么 | 承载这一系列的哪一层 |
|---|---|---|
| **包** `D:\Unity_Fork\HoUnityTools`（分支 `master`） | Unity 包 `com.hollow.hounitytools`。**主本在这儿** | 本文整份规范；`Runtime/FaceTracking/`（求值器 / 中间层 / 通道名 / 动态参数那对：Hub + Connector）；`Editor/FaceTracking/`（面板 / 会话 / `Profiles/*.hoface.json`）；`Tests~/` |
