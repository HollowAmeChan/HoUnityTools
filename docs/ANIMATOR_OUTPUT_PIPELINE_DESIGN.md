...
> 2. **组件值不直接写其它组件**，而是写进角色上的 **`HoFaceSemanticHub`**（中控组件：`float[]` + 下标）。
...
| **Hub 语义槽** | **下标 + 名字 + 中性值 + 范围** | 按下标写 `hub.values[i]`，**不反射** |
...
方向 2（导出时重绑到纯数据代理）**正是 Hub 的形态** —— 影子上的那份 Hub 只有值、没有资产
（`asset == null` 时 `Count` 退化成数组长度、`KeyAt` 返回空串），
