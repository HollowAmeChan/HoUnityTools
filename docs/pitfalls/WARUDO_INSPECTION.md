...
**⚠️ 顺带一个"显示"上的坑：`object` 口拿到数组时，`ToString()` 只给你类型名。**
用户实测：控制求解（当时还叫「Ho Face 处理链」）的 `Bone Rotations`（`Quaternion[]`）接到「Ho调试日志」，屏上只有一行
`UnityEngine.Quaternion[]`；而同一个口接到官方「检查值」**能出值** —— 因为官方那句是
`JsonConvert.SerializeObject(A)`（**序列化**，数组自然展开），我们只有 `ToString()`。
所以"官方的能出值、我们的不能"这种对比，**先看显示方式，再看接线**。
修法就是在自己的 `Describe` 里认这几类：字符串原样、名字→值的表（排序摊平）、
**数组/列表逐项**（`[i] = (x, y, z, w)`，`T[]` 与 `List<T>` 都实现 `IList`）、
`Vector3`/`Quaternion` 显式 F3；其余才回落到 `ToString()`。
...
副作用心里要有数：直读等于**替流程图求值一次上游那个口**。三条都是源码级确认过的：
① 上游每被读一次就得多算一次 —— 我们接收器的 `RawValues()` 是 `HoFaceInputState.Snapshot()`，
**每调一次新建一个字典**（源码注释原话"52~200 个键拷一份"），而控制求解这一层本来就要一次，**这是额外的第二次**；
② 我们自己每读一次还要把整张表摊成文本（排 65 个键 + 拼 ~1.3 KB 字符串）**只为比"变没变"**；
③ 每读一次还多一次 `Graph.GetInputDataConnections(this)`。
→ 定案：**不是每帧读**，而是每 `ReadInterval`（默认 **0.1 s = 10 Hz**）读一次 —— 观感没差别、垃圾少 6 倍。
**要看每一帧的值就用官方「查看值」**（它只读自己的字段、不替谁求值）。
还有一条语义上的：**求值时机由我们决定**（我们的 `OnUpdate`），不是流程图决定 —— 我们自己的口都是现场算的
纯读、控制求解的 `Ensure()` 还有 `Time.frameCount` 护栏，所以没有可观察后果；
**但把有副作用的口接进来，就等于让我们替你触发了它**。
