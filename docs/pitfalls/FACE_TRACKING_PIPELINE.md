...
原因：`HoFaceDebugSettings.Inputs()` 的语义是**只认配置文件的 `inputs`**。配置文件里没有 `inputs` 字段就是**空表** ——
一条输入行都没有，通道层自然拿不到任何值。（**没指定配置文件也是空表**：**留空 = 空表、这一层不做事**，
**没有内置默认兜底** —— 2026-09-25 起与 Warudo 侧统一；这是刻意的，不做隐式兜底。）
