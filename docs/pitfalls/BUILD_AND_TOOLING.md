...
### 9.1 ⚠️ 症状的**另一面**：接收器没起来时，图照跑、参数全是 0（2026-09-26 现场又踩一次）

热更新把接收器关掉之后，**下游不会报任何错**：中间层节点照样输出**整张键表**，
只是**每个值都是 0**（"缺键 = 这一行不写、保持上一帧"，而初始值就是 0）。
于是现象看起来是"同样的配置，突然输出不出来了"，很容易怀疑到中间层/控制器头上。

**一眼分清的判据**（都在 `Player.log` 里，不用截图）：

| 看到 | 意思 |
|---|---|
| `[Ho 面捕] 插件被卸载：接收器已关（下次点 Connect 会重新绑端口）。` | 刚刚热更新过一次 ⇒ **必须重新点 Connect** |
| `[Ho 面捕] 接收器 运行中=False  状态=未启动  本帧键=0`（之后**再没有**新的接收器行） | 接收器确实没在跑 |
| `[Ho 面捕] Connect（手机模式）→ …` 在热更新之后**没有出现过** | 你还没点（点过的话一定会有这一行） |
| 调试节点：`还没有值：直读 「HoFaceVTS接收器」.RawValues = 0 个键 · 端口 = 空` | 原始值表是空的 |
| 参数处理 `状态`：`原始键 0（不新鲜）` | 上游一个线名都没来过（对照：来了但全是 0 时这里会是 `原始键 90（新鲜）`） |
| 参数处理输出被 dump 成一整张 `= 0.000` | 就是上面那条的必然结果，**不是**中间层坏了 |

**怎么办**：热更新之后重新点一次接收器的 **Connect**（`状态` 变成 `运行中=True`、`本帧键>0`），
再看 `FaceFound` 有没有值 —— 手机没看到脸时它也会是 0，那是来源的事，不是链路的事。
