...
| `HoFace调试日志` | **正式（给人看的小工具）** | **形状照官方「查看值」抄**：`[DataInput] object A`（什么类型都能接）+ `[Markdown] string Text`（只读文本块）+ `OnUpdate` 里变了才重写并广播，**只多一个 `[Trigger] 复制到剪贴板`**（`GUIUtility.systemCopyBuffer`）。踩过的两版：`[Disabled]` 的口收不到上游写入；"可编辑多行框当显示行"界面不刷 |
