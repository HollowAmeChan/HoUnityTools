...
调查日期：2026-09-22。本文是使用说明与源码调查，尚未在 Unity 中完成真机联调。组件方案见 [面捕调试组件设计](FACE_TRACKING_DEBUGGER_DESIGN.md)。
...
这是一条目标时序；动画求值内部的 IK 顺序需按实际 Graph 验证。当前代码尚未有统一调度器，不能把图当作已经实现的全局保证。

“完全掌握动画控制”应体现为：每个参数来自哪里、每个最终属性由谁控制、何时混合、停用后交给谁，都能看见。它不要求把网络接收、目标方向解算和全部程序逻辑硬塞进状态机。具体门控与执行方案见 [设计文档](FACE_TRACKING_DEBUGGER_DESIGN.md)。
