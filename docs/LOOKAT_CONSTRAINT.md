...
| `eyeWeight` | `0..1` | **0.3** | 眼球强度（两种模式共用）：0 = 眼睛完全不动。实测 0.3 最自然 |
...
| `bodyWeight` | `0..1` | 0.3 | 脊椎 | 面板叫「身体强度」。**= Unity 的 `bodyWeight`**：身体参与度（Unity 沿脊柱分摊）—— 这就是"带着身子动" |
| `spineMinAngle` | `float` | 0° | 高级 | 只在总角度超过它之后才开始分摊（0 = 一直参与） |
| `headEnabled` | bool | true | 头颈 | 是否转头/颈 |
| `headWeight` | `0..1` | 1 | 头颈 | 面板叫「头部强度」。**= Unity 的 `headWeight`**：头部参与度（0 = 只做眼睛） |
...
| `headShare` | `0..1` | 0.7 | 头颈 | 面板叫「头部承担」：超过死区之后头部承担的比例（剩下的给眼睛） |
| `deadZone` | `float` | 8° | 头颈 | 面板叫「起始死区」：这么小的角度只用眼睛 |
