...
"要不要进动画状态机"其实是**两件成本差一个量级的事**，建议分开决定。

### (a) 被状态机或图驱动（现在就该预留接口，几乎零成本）

我们的效果作为**运行时**节点，由 Animator 状态、Warudo Blueprint/节点图或 Timeline 曲线控制它的**权重与目标**。要做的只有公开一组可被外部写的接口；不预留的话以后要动序列化结构：
...
### (b) 烘焙成剪辑交给状态机/时间轴（本版不做，但设计上已经兼容）

这是真正意义上的"进动画状态机"：把效果烘成 `AnimationClip`，之后完全不依赖运行时。

**可行性比布料好得多**：我们的输出是**骨骼旋转（humanoid 身体）+ 形态键值**，两者都是标准的可动画属性 —— 这个效果**天生可烘焙**（MC2 布料那类模拟烘不了）。

- Warudo 里对应的路径是 **Character Animation Mod**：humanoid 骨骼剪辑改名 `Animation`、放进 mod 文件夹、`Warudo → Build Mod`，产物丢进 `CharacterAnimations`（[Warudo Handbook](https://docs.warudo.app/docs/modding/character-animation-mod)）。⚠️ 那份文档明确提醒"动画里有**非骨骼 Transform 或材质属性**要走别的办法"，而**形态键曲线能否跟着这条路走没有写明，需要实测**（眼睛那一路可能得改走 Warudo 的 expression / BlendShape 驱动）。
- **鼠标模式不可烘**（交互式）；物体模式 + 固定动作可以。
- 烘焙采样必须在"头部 IK 已应用、眼睛已写入"之后（即我们第二段之后 / `PostLateUpdate`），否则烘出来的姿势与运行时对不上。
- 烘焙完必须**禁用组件**，否则两份数据打架。

### 现在就要守住的三条"可烘焙性"约束

1. **每帧输出只依赖"目标点 + 参数"**，不依赖历史（平滑器/弹簧除外，烘焙时一并采样即可）。
2. **只写标准可动画属性**：骨骼旋转（由 Unity IK 写）与 `SkinnedMeshRenderer` 形态键；不碰材质、不碰非骨骼 Transform（正好也是 Warudo 那条路的限制）。
3. **姿势只在固定时机改**：头部在 `OnAnimatorIK`、眼睛在 `LateUpdate`，采样点固定可预期。

守住这三条，将来加烘焙工具就只是"录一段 + 存 clip"，不用动现有结构。
...
2. **三块骨架 + 面板**：①/②/③ 三块的启用与权重、目标区、Animator 三项检查；此时可以先只让眼睛块工作。
...
- **烘焙成剪辑**：见「以后的事」——（b）那条路，等这套跑顺了再做（形态键能否进 Warudo 的 Character Animation Mod 需要先实测）。
- **状态机/图驱动组件**：一个 `StateMachineBehaviour`（按状态设权重/目标）与 Warudo Blueprint 节点；前置是 (a) 的公开接口。
...
4. **三块的划分**：① 脊椎 / ② 头颈 / ③ 眼睛 这样够不够？要不要把"② 头颈"再拆成"颈"和"头"（那样就得自己写分摊，回到 Unity IK 之外的路线）？我建议保持三块。
5. ⚠️ **确认你的角色运行时有 humanoid Avatar**：`potato_build.prefab` 里 `Animator.m_Avatar = 0`。humanoid 路线下没有 Avatar 时**头部与脊椎完全不生效**（面板会报错），眼睛形态键部分照常工作。
6. **(a) 的外部接口现在就要吗？**（`Weight`/`SetTarget`/`SetMode`）我建议这一版就留，成本极低；`StateMachineBehaviour` 组件与烘焙工具都放到后续。
