...
- 鼠标模式移出窗口 / 失焦 / 设备没数据：`mouseOffscreenBehavior`（默认 `Hold` = 保持最后方向）。
  ⚠️ Input System 在这些情况下会把鼠标位置报成 **(0,0)** —— 如果直接采信，表现就是"丢失跟踪后视线突然跳到画面角落"，
  而且丢失判定永远不触发。所以 **(0,0) 一律当作"没有指针数据"**（鼠标真停在窗口左下角那个像素上的概率可以忽略），
  此时按丢失处理：`Hold` 保持最后方向，否则走 `lostBehavior`。屏幕叠加会显示「指针不可用→按丢失处理」。
...
| 分组 | 面板可见 | 高级里 | 说明 |
| --- | --- | --- | --- |
| 看哪里 | 5~8 | 0 | 模式、目标+偏移，或 相机+取法+（摇杆：灵敏度/死区/坐标系 · 射线：层/兜底距离）+离屏保持。鼠标那几项全在这一区、按取法显隐 |
