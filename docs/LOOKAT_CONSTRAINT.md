...
7. **眼睛补的是"实际头部姿势"**：`OnAnimatorIK` 里在调用 `SetLookAtWeight` 之前记下头骨姿态，`LateUpdate` 里用"IK 后的姿势 ÷ IK 前的姿势"这个旋转增量量出**头部实际转了多少**，眼睛残余 = 目标角 − 实际头角。这样"头 + 眼睛"恒等于目标方向。
...
| 来源 | 症状 | 修法 |
| --- | --- | --- |
| **眼睛补的是命令值，不是实际值** | Unity 的 LookAt IK 是近似解（`headWeight`、肌肉范围、脖子分摊都会让它到不了位），头少转多少眼睛就偏多少；**「只看眼睛」时最夸张** —— 头一点没动，眼睛却只补了 `1 − headShare`（三成） | 眼睛残余改为 `目标角 − 头部实际角`（量骨骼姿势增量，跟骨骼自身轴向无关）。头没转到位就由眼睛补齐，`headWeight` 设多少都不影响落点 |
...
**自检**：调试区有 `目光误差 = 目标角 −（头实际 + 眼睛）` 一行，强度拉满、没被限位夹住时应该接近 0（面板会标「精确」）；Scene Gizmo 里紫线（实际目光）压住黄线（目标）也是同一个意思。误差大时先看面板上「头命令 vs 头实际」差多少 —— 差得多说明 IK 没做到位（眼睛会补上，但头看起来不够转），那就调 `headWeight` 或检查 IK Pass。
...
| 在哪个视图调试 | 怎么用 | 看什么 |
| --- | --- | --- |
| **Game 视图**（鼠标的正常用法） | 「调试 → Game 视图叠加」默认开 | 屏幕上直接叠读数：青十字 = 鼠标位置、黄点 = 目标、紫点 = 实际目光落点、两点之间的连线就是偏差；左上角一行完整读数（总角度 / 头实际 / 眼 / 目光误差 / 驱动模式），误差 < 1° 会标「精确」 |
...
| 头部承担方向 | 青 | 头部**实际**转到哪（不是命令值），带 "头实际 xx° / xx°" 标签 |
| 实际目光方向 | 紫 | **头实际 + 眼睛残余** = 真实目光，"目光 xx° / xx°" 标签；紫线压不住黄线就是偏了 |
...
| 眼睛总差一点才指到鼠标 | Game 视图叠加里"青十字（鼠标）到紫点（目光）"的连线，或调试区的「目光误差」 | 连线是斜的、且随角度变大 → 先看「头实际」与「头命令」差多少（IK 没到位，眼睛已补，属正常）；连线固定往中心缩 → 眼球限位夹住了（调 `eyeBoneLimitYaw/Pitch`）；`eyeWeight` 没到 1 也会按比例缩 |\n| 眼动键总是 100，眼睛像"定住"了 | 调试区条形 + 饱和清单 | 总角度已经超过该通道的角度上限（正常吃满）→ 调大角度上限或压眼球强度；若角度还没到上限就已经 100，说明有眨眼/高光/表情在写同一个键 → 换「合并方式」 |
