...
| `mouseSampleMode` | `CursorPoint / AngleMap / Raycast` | **CursorPoint（准星）** | 看哪里 | 鼠标取法（见「鼠标模式」）。默认准星 = 眼睛正好落在鼠标指的位置 |
...
| **鼠标"角度摇杆"不是"看向鼠标指的点"** | 灵敏度是人为的线性映射（跟相机 FOV 无关），角色不在画面中心时还有视差 —— 眼睛方向对了，但没落在鼠标那个点上 | 默认改成**准星**（`CursorPoint`）：取鼠标射线上"和角色一样远"的点当目标点，眼睛正好落在鼠标指的位置 |
...
| 取法 | 做法 | 什么时候用 |
| --- | --- | --- |
| `CursorPoint`（默认，准星） | 鼠标射线在**角色所在深度**上的那个点当目标点（支点在射线上的投影深度） | **想要"眼睛精确落在鼠标指的位置"就用它**：方向由支点指向那个点，指哪看哪，跟相机 FOV、角色在不在画面中心都无关 |
| `AngleMap`（角度摇杆） | 鼠标屏幕位置 → 归一化 (−1..1) → 乘 `mouseSensitivity` 得到 yaw/pitch 偏移（中心死区可调） | 想要"鼠标即摇杆"的手感（灵敏度自己定，不看相机）；注意它**不等于**看向鼠标那个点，角色不在画面中心时会有视差 |
| `Raycast` | 相机 → 鼠标射线打到 `mouseRaycastMask` 的物体；**没打中时取射线上 `mouseDistance` 处的点** | 有场景几何、想让角色盯着墙上的东西 |

> v1 的 `WorldPoint`（固定距离 / 投到水平面）已删除；它的"看向鼠标位置"意图现在由 `CursorPoint` 更好地实现（不需要用户填距离）。
