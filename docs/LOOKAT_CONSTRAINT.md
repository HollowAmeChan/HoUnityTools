...
| `eyeWeight` | `0..1` | 1 | 眼球强度（两种模式共用）：0 = 眼睛完全不动 |
| `eyeBoneLimitYaw` / `eyeBoneLimitPitch` | 度 | 35 / 25 | 眼球最多往左右 / 上下各转多少。正常扫视很少超过 30°，超过就夹住，避免翻白眼 |
...
| --- | --- | --- |
| `horizontalInner`（**往右**） | 目标角度上限（默认 30°） | 该角度下的满值输出：形态键 = 0..100，骨骼 = 0..30° |
| `horizontalOuter`（**往左**） | 同上（左右可以给不同上限，很多模型外侧能动得更多） | 同上 |
| `verticalUp`（**看上**） | 默认 20° | 同上 |
| `verticalDown`（**看下**） | 默认 25° | 同上 |
...
| `eyeAngleLimit` | `Vector4` | (30°, 30°, 20°, 25°) | ③ | 四条曲线横轴对应的角度上限（往右/往左/上/下） |
| `horizontalInner/Outer`、`verticalUp/Down` | `AnimationCurve` × 4 | 线性 | ③ | 四条方向曲线（见上） |
| `eyeEntries` | `List<HoLookAtEyeEntry>` | 空 | ③ | 四条通道（键名 + 增益），两格同名只写一次 |
| `eyeDriver` | `EyeBones / ShapeKeys` | **EyeBones** | ③ | 驱动方式：眼球骨骼（默认）或形态键，二选一不混用 |
...
4. **③ 眼睛 · 形态键**：四条方向曲线 → 往右/往左/看上/看下四向键输出；两族键名约定由预设自动填（同一套通道）。验收：左右移动时"往右/往左"互斥切换、垂直不受影响；把某条曲线横轴上限 30°→15°，输出幅值随之翻倍。
5. **③ 眼睛 · 眼球骨骼（v4 起是默认模式）+ 一键装配**：`GetBoneTransform(LeftEye/RightEye)` + 世界空间叠加，残余角直接当转角用（精确指向，不过曲线）。验收：眼球骨骼自身轴向任意旋转 90° 后看向方向不变；新模型点一次"一键装配"即可用（有骨骼就自动选骨骼模式）。
...
3. **四条曲线的角度上限默认值**：往右/往左/上/下 = 30°/30°/20°/25°。要不要在调试区加一个**标定**按钮（拖动虚拟目标把某个方向推到形态键满值，记录当时的实际角度并回填上限）？
