...
| `headDirectionTrim` | `Vector2` | (0, 0) | ② | 面板叫「头朝向偏差」：模型静止姿势的头不朝正前方时那个固定差值，眼睛会跟着补正。正常模型 0 |
...
| `eyeAngleLimit` | `Vector4` | (15°, 15°, 10°, 10°) | ③ | 四条曲线横轴对应的角度上限（往右/往左/上/下） |
...
- **所有分区默认折叠**：面板打开就是一张目录（看哪里 / 角色 / ①②③ / 丢失与瞬移 / 高级 / 调试），要看哪块点哪块；每个分区标题右侧都有摘要（强度、模式、有无目标等），不用展开也能看个大概。
