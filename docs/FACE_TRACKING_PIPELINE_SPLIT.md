...
**所以中间层该不该产生"摆锤物理参数"？该，但不是让中间层自己实现摆锤** —— 那会变成第二个 `HoPendulumConstraint`。正确的定位是：

> **中间层是「参数来源的汇合 + 整形」，物理只是其中一路来源，和"手机输入"并列。**

缺的不是物理实现（已经有了），是**"把物理输出接进面捕参数链"这条通路**。这条通路在 Warudo 那边天然就是"摆锤节点 → 平滑节点"，和我们四段流水线的 ② 对得上。
