...
日期：2026-09-22（§5/§6 于 2026-09-23 按现状重写，§5 在删掉生成器后改成"搬运"模型）。
本文是**源码实测 + 一次判别性实验**的记录：参考实现怎么搭的、我们怎么搬运一份控制器、
...
## 5. 现在的做法：控制器是**作品**，初始化只负责搬运（2026-09-23 改）

**代码不再生成控制器。** 一份面部控制器（Jerry 的 `vrc-common`、我们自己编的 `ho-2d-test1`…）
在 Unity 的混合树编辑器里被编出来，**自带片段与动画**，它自己就是真源。初始化只有一件事：

```
源控制器（任意一份 .controller）
  ↓ ① 整份复制到目标路径（层、状态、参数、子树、子片段全跟着走）
  ↓ ② 把每条形态键曲线重绑到「驱动对象」列表里的网格上
  ↓ ③ 源里引用的**外部片段**先复制成本文件的子资产再改（绝不改别人共享的 .anim）
目标角色的面部控制器
```

| 项 | 现状 |
| --- | --- |
| 图层 / 状态 / 参数 | **跟着源走**，我们不增不减（门控、轴这些名字是源里就有的） |
| 驱动对象 | 组件上的**网格列表**；曲线按"列出的网格里谁有这个键"摊开，一个键可以落在多个网格上 |
| 模型上没有的键 | 那条曲线**原样留着不动**（作者摆的格子数据不丢），但结构摘要会报出"这些格子落不到任何网格上" |
| 覆盖式重新初始化 | 文件整个换掉，**GUID 不变** —— 场景里引用它的地方（窥视对象的 Animator）不会断 |
| 源控制器 | **一个字节都不改**；它引用的外部 `.anim` 也一样（复制出来再改） |

**为什么把生成器删了**：那等于在代码里重新发明一遍混合树编辑器，还得把"轴的值怎么算"这种
中间层的事写进控制器。而控制器**不需要声明它吃什么样的参数** —— 它只等着被喂
（见 [面捕工作流](FACE_TRACKING_WORKFLOW.md) §1）。
代价是诚实的：**我们不再能保证控制器的形状**，所以面板改成**从资产读实况**
（几层几个状态、会写哪些键、哪些键这台模型没有），而不是从配置推断。

**边界仍在**：运行期只允许形态键曲线；无 Behaviour、无同步图层、无事件/对象曲线（`Compile` 会拦）。
搬来的控制器带了这些时，初始化不拦，但**开始驱动时**会被拒，面板会说是哪一条。

### 5.1 搬运之后，谁负责什么

| 东西 | 谁管 |
| --- | --- |
| 树形、格子坐标、每格写哪个键写多少、门控与轴参数的名字 | **控制器作品**（作者在混合树编辑器里定） |
| 动画驱动哪些网格 | 组件上的「驱动对象」列表（初始化时作用于曲线绑定） |
...
**幂等**：重绑是幂等的（反复初始化结果一致）。驱动对象列表变了就再初始化一次，曲线跟着列表走；
把某个网格从列表里去掉，它上面的绑定也就没了 —— 键在别的网格上时不会留下孤儿绑定。

**下面 §5.2 那张表不是"我们现在生成的形状"**，而是**编控制器时的作者约定**（我们上一版生成器
就是照它出的，所以留着当参考）。这个仓库里没有 `ho-2d-test1.controller`：那份作品在作者手上，
仓库只负责搬运它。

### 5.2 命名规则与对照表（**作者约定**，不是代码规则）

> 代码不再生产这些名字了（§5）：格子名与树名活在**控制器作品**里。这份约定留给编控制器的人 ——
> `ho-2d-test1` 那份作品按它编，以后新编的树也照它编。**运行期只有参数名还被代码引用**
...
**归属不靠名字判断**：整份文件都是搬过来的，重绑按"驱动对象里谁有这个键"做 ——
所以改名字不会漏掉任何东西（名字是作者那侧的数据，搬运照样按键名对得上）。

## 6. 参考实现里我们抄了 / 还没抄的

| 参考实现的东西 | 我们 | 落在哪 |
| --- | --- | --- |
| **二维区域**（眼睑 `开合 × 眯眼` 五格姿势） | ✅ 抄了（我们那份作品的六格是它的直系后代） | 控制器作品（§5） |
| **区域门控**（`EyeTrackingActive` / `LipTrackingActive`） | ✅ 抄了 | `Ho/Drive/Gate/*` 参数 + 两棵区域子树（作品里） |
...
| **耦合**：`SmileFrown` 同时驱动嘴角 / 脸颊 / 酒窝 | ✅ 已确认（资产A L207-218、L417-428） | 我们**没抄**：耦合留在中间层 / 控制器作品里 |
...
| **眼球**：一棵 2D 树管四方向，坐标 **±0.7**（中性 `(0,0)`） | ✅ 已确认五姿势；Shinano 加对角共 **9 姿势** | ❌ 还没做（要加就加在控制器作品里，§6.1 有完整形状） |
...
面板的「控制器结构」会把它们列出来（搬运时曲线原样留着，不删）。
...
# （同一份用例也覆盖搬运：整份复制、外部片段复制、重绑驱动对象、覆盖不改 GUID）
```
