...
| **中间层输出阶段** | 用户想"用一个值把某个门控关掉"时，**不去改控制器的默认值**，而是在**参数写进控制器之前**把那个键覆盖进字典（`HoStringFloatMerge` 的 `覆盖` 口接一份「HoStringFloatDict」手填的表：一行 `Ho/Drive/Gate/Lip = 0`） | 覆盖发生在**同一次写入**里，不会多出"第二个写者"在旁边抢；也不用往配置文件里再加"gate 段 + 默认值"这套机制 |
