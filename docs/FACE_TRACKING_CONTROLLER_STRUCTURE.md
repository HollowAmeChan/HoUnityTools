...
1D 树（只有一根轴）去掉 y 的全部字段：`<树>_<x语义>__<格语义>__<x格数>_<x索引>`。

| 字段 | 规则 |
| --- | --- |
| `<树>` | `LidL` / `LidR` / …。**必须有**：左右两棵树的轴语义、格语义、格点完全一样，不带侧就会重名 |
| `<x语义>` `<y语义>` | **轴名 = 正端语义 + 负端语义**，`+1` 的那端在前。眼睑开合是「闭 / 睁大」→ `BlinkWide`；单端轴只写正端（眯眼 → `Squint`） |
| `<格语义>` | 各轴的**非中性端**按 x→y 顺序直接拼接（`WideSquint` / `BlinkSquint`）；两端都中性写 `Neutral` |
| `<x格数>x<y格数>` | **格子数**（3 列写 `3`），不是最大索引 |
| `<x索引>_<y索引>` | 这一格的索引，**从 0 开始** |
| `__` | 唯一的结构分隔符 —— 轴语义里不许出现它 |

**片段名里不带具体键名**（格式没给键名留位置）。这就是下面这张表存在的原因。

**眼睑 2D 对照表**（左眼；右眼同构，`LidL`→`LidR`、键名 `Left`→`Right`）：

| 格点 | 轴值（开合 / 眯眼） | 片段名 | 这一格写的键与值 |
| --- | --- | --- | --- |
| `0_0` | −1 / 0 | `LidL_BlinkWide_Squint__Wide__3x2_0_0` | `eyeWideLeft 100` |
| `1_0` | 0 / 0 | `LidL_BlinkWide_Squint__Neutral__3x2_1_0` | 全 0 |
| `2_0` | +1 / 0 | `LidL_BlinkWide_Squint__Blink__3x2_2_0` | `eyeBlinkLeft 100` |
| `1_1` | 0 / +1 | `LidL_BlinkWide_Squint__Squint__3x2_1_1` | `eyeBlinkLeft 90` + `eyeSquintLeft 100` |
| `0_1` | −1 / +1 | `LidL_BlinkWide_Squint__WideSquint__3x2_0_1` | `eyeWideLeft 100` + `eyeSquintLeft 100` |
| `2_1` | +1 / +1 | `LidL_BlinkWide_Squint__BlinkSquint__3x2_2_1` | `eyeBlinkLeft 100` |

**第四行的 `blink 90` 是这张表最有价值的一条**：眯眼自带眨眼量，所以"又眨又眯"永远不会加和成
200 —— 名字里看不出来（名字只说"哪一格"），必须查表。补 `2_1` 那格的**艺术决定**同理：
"眨满时眯眼还剩多少"。
...
| 眼睑两根轴参数 | `Ho/Drive/Lid/Left/BlinkWide`、`…/Squint` | **轴语义与片段名同一批词**，所以"名字 → 参数"是机械映射，不用查表 |
...
| 轴 | +1 / −1 | 轴名 | 网格 | 格语义词 |
| --- | --- | --- | --- | --- |
| 眼睑开合 | 闭 / 睁大 | `BlinkWide` | `3x2` | `Wide` `Neutral` `Blink` `Squint` `WideSquint` `BlinkSquint` |
| 眼睑眯眼 | 眯 / 无 | `Squint` | （同上，Y 轴） | |
| 凝视内外 | 内 / 外 | `InOut` | 未来 | `In` `Neutral` `Out` |
| 凝视上下 | 上 / 下 | `UpDown` | 未来 | `Up` `Neutral` `Down` |
| 唇角 | 欣 / 悲 | `SmileFrown` | 未来 1D | `Smile` `Neutral` `Frown` |
| 横向 | 右 / 左 | `RightLeft` | 未来 1D | `Right` `Neutral` `Left` |
