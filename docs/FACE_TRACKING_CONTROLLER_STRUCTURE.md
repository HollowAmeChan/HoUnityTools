...
| **中间层输出阶段** | 用户想"用一个值把某个门控关掉"时，**不去改控制器的默认值**，而是在**参数写进控制器之前**把那个键覆盖进字典（`Ho合并字典` 的 `覆盖` 口接一份「HoStringFloat」手填的表：一行 `Ho/Drive/Gate/Lip = 0`） | 覆盖发生在**同一次写入**里，不会多出"第二个写者"在旁边抢；也不用往配置文件里再加"gate 段 + 默认值"这套机制 |
...
| 树的形状 | "有效门控"是什么 | 传得出来吗 |
| --- | --- | --- |
| **一层**（现在这份控制器就是）：根 Direct 的子节点各自挂一个 gate 参数 —— `Ho/00 Drive Tree` 的 4 个子节点 = `Ho/Drive/Gate/Eye`×3（LidL / LidR / EyeRegion）+ `Ho/Drive/Gate/Lip`×1（LipRegion），两个 gate 都是 `m_DefaultFloat: 1` 的 float 参数 | **就是那个参数** | ✅ 想让它在 Hub 上就加一行输出行（或经「HoStringFloat」+合并字典覆盖）；一层时它就是真值，**下游不用重算** |
