...
| **中间层输出阶段** | 用户想"用一个值把某个门控关掉"时，**不去改控制器的默认值**，而是在**参数写进控制器之前**把那个键覆盖进字典（`Ho合并字典` 节点：`覆盖行` 里填一行 `Ho/Drive/Gate/Lip = 0`） | 覆盖发生在**同一次写入**里，不会多出"第二个写者"在旁边抢；也不用往配置文件里再加"gate 段 + 默认值"这套机制 |
...
这是"动态参数只有一个写者（中间层）"带来的**唯一硬边界**：Hub 里装的永远是**中间层算出来的参数值**，
**不是"树算出来的有效权重"**。它在门控上露出来：

| 树的形状 | "有效门控"是什么 | Hub 传得出来吗 |
| --- | --- | --- |
| **一层**（现在这份控制器就是）：根 Direct 的子节点各自挂一个 gate 参数 —— `Ho/00 Drive Tree` 的 4 个子节点 = `Ho/Drive/Gate/Eye`×3（LidL / LidR / EyeRegion）+ `Ho/Drive/Gate/Lip`×1（LipRegion），两个 gate 都是 `m_DefaultFloat: 1` 的 float 参数 | **就是那个参数** | ✅ 想让它在 Hub 上就加一行输出行（或用合并字典的覆盖行）；一层时它就是真值，**下游不用重算** |
| **嵌套**：门控下面再挂门控（子区域开关 / 用乘法把两个开关合成"有效门控"） | 各层相乘，只活在树里 | ❌ **传不出来** —— 下游要真值得自己按同样规则再乘一遍（而"控制器里再算一遍"那条路 2026-09-26 已经删了，不打算加回来） |
