...
| **动画文件夹**（动画工具的「形态键动画」栏，`HoUnityTools/动画工具` / 手 K） | 姿势数据 | 一堆 `.anim`，文件名 = 槽位名（比如每个形态键一份 `<键名>.anim`） |
...
§3.2 说的是"我们**不注入**门控"。那**用户想关掉某一块**怎么办？现在的答案分两半：

| 谁 | 干什么 | 为什么 |
| --- | --- | --- |
| **树侧（控制器）** | **门控只门控"动作"**：恒 1 的门控参数画在**使用者自己的树 / 子节点**上，默认值住在**控制器参数**上（我们那份实测控制器里就是 `m_DefaultFloat: 1`） | 门控本质是**空间混合**，混合树就是干这个的；中间层不该知道树里有哪几个门 |
| **中间层输出阶段** | 用户想"用一个值把某个门控关掉"时，**不去改控制器的默认值**，而是在**参数写进控制器之前**把那个键覆盖进字典（`HoStringFloatMerge` 的 `覆盖` 口接一份「HoStringFloatDict」手填的表：一行 `Ho/Drive/Gate/Lip = 0`；也可以走 `HoStringFloat` + `HoStringFloatAppend` 的键值对链） | 覆盖发生在**同一次写入**里，不会多出"第二个写者"在旁边抢；也不用往配置文件里再加"gate 段 + 默认值"这套机制 |

要认的三条后果：

1. **门控不门控"参数"，只门控"动作"。** 中间层照旧不求值 gate —— Animator 参数与动态参数 Hub 里读到的
   仍然是**未门控**的值，下游要门控就自己门控。
2. **覆盖必须每帧给值。** Animator 参数会**保持上次写的值**，"这一帧字典里没有这个键" ≠ 回到默认值；
   要 1 就写 1（`HoStringFloatMerge` 的文件头注释里也写了这条）。
3. **名字必须在控制器参数表里**，否则**静默忽略**（写入那一步是按控制器声明的口逐个查字典的）。

#### ⚠️ 边界：**树里的门控只做一层**（2026-09-26 用户定）
...
| 树的形状 | "有效门控"是什么 | 传得出来吗 |
| --- | --- | --- |
| **一层**（现在这份控制器就是）：根 Direct 的子节点各自挂一个 gate 参数 —— `Ho/00 Drive Tree` 的 4 个子节点 = `Ho/Drive/Gate/Eye`×3（LidL / LidR / EyeRegion）+ `Ho/Drive/Gate/Lip`×1（LipRegion），两个 gate 都是 `m_DefaultFloat: 1` 的 float 参数 | **就是那个参数** | ✅ 想让它在 Hub 上就加一行输出行（或经「HoStringFloatDict」+`HoStringFloatMerge` 覆盖）；一层时它就是真值，**下游不用重算** |
| **嵌套**：门控下面再挂门控（子区域开关 / 用乘法把两个开关合成"有效门控"） | 各层相乘，只活在树里 | ❌ **我们那两条入口都交不出来**（clip 不行、我们也不往控制器里放代码）；真要就得**自己写一个组件**读 Animator 参数、算完再按名字写 Hub —— 那正是"脚本工作台"允许的事 |

**规矩**：**树 gate 只做基础 gate，只做一层，不做乘法分叉、不做嵌套的区域开关。**
想要"两个开关的与"（例如 `眼开 && 某条件`），正确做法是**在中间层乘好**，
把 `gate = a * b` 作为**一行输出**喂进树 —— 这样 Hub 里天然就是有效值，边界不成立。

⚠️ **别把"区域子树本身是 Direct"误判成违规。** 那是对的形状：区域子树里挂的是**每键的参数**
...
> 用户想临时关掉某一块，仍然走**合并字典的 `覆盖` 口**（同一次写入里改值，不产生第二个写者）。
...
> ⚠️ **名字的权威表在 [面捕命名权威](FACE_TRACKING_NAMING.md)**（2026-09-27 起）：42 个树名、轴段词、参数/门/切片/槽位
> 全在那一份里定，**本节只讲"名字的形状与为什么"**（四段式、方阵、左下原点这些），不再自己列名字。
...
⚠️ 这张表是**当年那几根轴**的速查；**完整的轴段词表（含 2026-09-27 新增的分侧轴）在
[面捕命名权威](FACE_TRACKING_NAMING.md) §6**：
...
| 横向偏嘴 | **+1 偏左** / −1 偏右 | `LeftRight` | 未来 1D | —— （旧版这里写 `RightLeft`，是没核公式的猜法：`mouthLeft − mouthRight + smileL − smileR` ⇒ 正端是**左**） |
