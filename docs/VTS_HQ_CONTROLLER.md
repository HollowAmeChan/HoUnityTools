...
| 4 | **槽位 114 个** | 🟡 **已各填一份「空片段」**（2026-09-27） | 片段在 `Assets/Hollow/土豆/FT/Animations/`，**文件名 = 槽位名 = 语义** ⇒ 混合树每格都显示名字而不是 None；空片段不写任何曲线 ⇒ **运行期行为与空 Motion 完全相同**。姿势还一条没摆 ⇒ **造型看不到**，但**轴值现在就能测**（§2.4）。刻度已按实测收过圈（`Open` 的 Y = `0/.4/.75`、`Form` 的 X = `0/+0.75/+1`、**`Roll` 的 = `0/.18/.45`（带 ±0.02 死区）**，负侧交给别的表；**右上角 `X2Y2` 单独挪到 0.6**） |
...
| **`MouthLipRoll`** | `Mouth/Roll`（轴带 **±0.02 死区**）**0 不卷 · 0.18 只卷嘴（不咬，= 猫嘴）· 0.45 咬唇最强** | —（**1D 表，没有 Y 轴**） | **3** | **"牙齿咬 + 嘴唇内卷"的程度**：这一个数就说完了（**不分上下唇**）；三档按实测收圈 —— 最强咬唇只到 0.45、光卷嘴 ≈0.18 | 只写**卷唇那几根键**（张嘴时这棵树照样生效 ⇒ 必须只动卷唇的键，别碰口孔） | `MouthLipRoll__Roll__A3X<i>` |
...
* ⚠️ **`MouthLipRoll` 是 1D**（2026-09-27 用户实测定）：原来拿 `mouthRollUpper` / `mouthRollLower` 两张 2D 轴，
  ...
  * **轴上有 ±0.02 死区**（2026-09-27 用户实测）：`|值| ≤ 0.02 → 0`、`0.02…0.05` 斜坡、之外**恒等**
    —— 和 `Mouth/Open` **同一条形状**（手机静息时就在这个区间抖）。实测刻度 `0.18 / 0.45` 原样过曲线，
...
| `Ho/Drive/Mouth/Roll` | `(mouthRollUpper + mouthRollLower) / 2` | **±0.02 死区 + 0.02..0.05 斜坡，之外恒等**（和 `Mouth/Open` 同形状） | smooth 0.009 s | **"牙齿咬 + 嘴唇内卷"的程度**（上下两根线的平均，**不分上下唇**）= 合同表 D 的 `HQUpperLipRoll`/`HQLowerLipRoll` **合并成一根轴**；喂 `MouthLipRoll`（**1D 表**）。三档按实测收圈：**0 不卷 / 0.18 只卷嘴（不咬，= 二次元猫嘴）/ 0.45 咬唇最强**。⭐ **张嘴时这根轴照样有值**（读数随张嘴降低但仍能到 0.18）⇒ 它就是为此独立成表的；闭嘴卷唇 = 最准的猫嘴。⚠️ 两根线本身没被删：`mouthRollUpper`/`mouthRollLower` 仍是 profile 的输入行与出口行，只是**控制器轴**不再各占一维 | **0 … 0.45**（0.18 = 只卷嘴 / 0.45 = 咬唇上限；静息噪声被死区抹平） |
...
2. 🟡 **按实测结果给 40 行加修饰符 / 修曲线**（§3.2 / §3.3）—— *你定数值*。已完成 / 待定：
   ...
   · ✅ **`MouthLipRoll` 从 2D 收成 1D，刻度再按实测收成 `0/.18/.45`，并加了 ±0.02 死区**（2026-09-27 用户实测定）：上下两根卷唇线一起增减 ⇒ 中间层平均成一根 `Mouth/Roll`；语义是"牙齿咬 + 内卷嘴的程度"（**不分上下唇**），三档 = 不卷 / 只卷嘴（**二次元猫嘴**）/ 咬唇最强（实测上限 0.45，够不到 1）。死区与 `Mouth/Open` 同形状（手机静息噪声）。**张嘴时这根轴照样有值**（读数降低但能到 0.18）⇒ 它就是为此独立成表；闭嘴卷唇是最准的猫嘴。文本生成器与 Unity 生成器两边同步改完，台架新增 3 条断言钉死死区与"刻度原样过曲线"。
