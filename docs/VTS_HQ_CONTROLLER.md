...
| 轴的位置 | Unity 的行为 |
| --- | --- |
| 在网格内部 | **精确双线性**（`Grid9` 在 (0.5,0.5) 读出 151.5 = 100×1.5+1.5，与手算逐位相同） |
| 超出网格（两侧都测了） | **钳到矩形边界那一行/列**（x=−3 与 x=−1 同值、x=3 与 x=1 同值）—— **不外推、不会超出子节点的值域** |
| **缺一格**（稀疏采样留空） | **不可预测、非单调**：实测到 `(1.5, −0.5, 0)` 这种**负权重** |

⇒ **"轴超了"最坏只是卡在边界姿势，不会画出畸形造型**。但有三件事得记着（详见
[能力边界](BLEND_TREE_LIMITS.md) §10 与[踩坑](pitfalls/BLEND_TREE_TRAPS.md) §6）：

1. **分辨率白扔**：`Mouth/Open` 实测只到 0.75，而槽位摆到 1 ⇒ 最后 1/4 的插值空间永远用不到。
2. **树与 Hub 会不一致**：钳制只发生在树内部，Hub / 面板读到的还是超出的原值（"Hub 在涨、姿势不动"）。
3. **缺角不能靠引擎兜**：稀疏摆法可以，但**每个能到达的角都要夹住**。

所以：**量程、死区、稳定都在中间层做**（§3.2），树里只负责"这个轴值长什么姿势"。
