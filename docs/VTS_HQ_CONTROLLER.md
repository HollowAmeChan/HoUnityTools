...
* ⭐ **卷唇 = 轴驱动的变体开关**（2026-09-27 用户定，取代了"残差车道"那版）：中间层把
  ...
  * ⭐ **怎么区分"上下同卷"和"噘嘴"（2026-09-27 四条 5 秒实测定）**：判据就是**两根 roll 线是不是一起动** ——
    ...
    另外第二道独立保险 = **噘嘴门** `1 − clamp((mouthPucker − 0.4)/0.3, 0, 1)`
    （静息噘嘴基线 0.26..0.35 不碰、噘嘴 0.975 掐死、真猫嘴 0.03 不碰）。
...
6. **标定卷唇的判据与增益**（§3.1，2026-09-27 改）：用「参数输出」栏的 **`记 5 秒`** 按钮各录一次，
   ...
   **③ 静息**（应 = 0.00）、**④ 张嘴卷唇**（应仍 ≥ 0.12；不够就加大表达式里那个 `4`）。
   ...
   噘嘴那道门是 `(mouthPucker − 0.4)/0.3`（静息基线约 0.26..0.35，别把阈值压到基线以下）。
...
| `Ho/Drive/Mouth/Pucker` | `((mouthDimpleRight + mouthDimpleLeft) * 2) - mouthPucker` | 恒等 −2…2 | smooth 0.007 s | **双向**：−1 噘嘴 … +1 展宽。⚠️ **实测（2026-09-27 五秒统计）：静息 ≈ −0.18（`mouthPucker` 的静息基线就有 0.26..0.35！）、噘嘴 ≈ −0.47、真猫嘴会冲到 +2 并**饱和**（卷唇时 dimple 也在 0.5 上下）| 静息 **−0.18** · 噘嘴 **−0.47** · 猫嘴 **+2（饱和）** |
