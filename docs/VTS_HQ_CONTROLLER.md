...
| 轴 | 静息 avg | 谁在用 | 判定 |
| --- | ---: | --- | --- |
| `Mouth/Form` | **−0.132** | `MouthCore` X | 负值钳到 X0 ⇒ 今天不出错；**成因是 `mouthPucker` 的基线**（公式里减了它） |
| `Mouth/Pucker` | **−0.177** | `MouthWidth` X | ⚠️ 静息就 18% 朝"噘/倒V"那侧 ⇒ **做倒V前必须归一** |
| `Brow/*/Y` | **−0.36** | `BrowCoreL/R` X（−1/+1） | ⚠️⚠️ 静息就 **36% 朝"压眉"** ⇒ 中性脸会被画成怒/严肃；待修 |
...
* ⚠️ `Lid/*`、`Gaze/*`、`Brow/*` 三行要看**干净静息**才作数 —— 要一次"**睁眼放松、盯镜头、嘴闭**"的 5 秒。
