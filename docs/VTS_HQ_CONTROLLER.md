...
| 4 | **槽位 119 个** | ⬜ **全是空 Motion** | 100% 未摆姿势 ⇒ **造型一条都还看不到**；但**轴值现在就能测**（§2.4）。**刻度已按实测收过一次圈**：`MouthCore` 的 Y（`Open`）= `0 / 0.4 / 0.75`（2026-09-27，见 §2.2 与 §2.3.1） |
...
* **不能测**：造型好不好看 —— 119 格全是空 Motion，插值出来的还是启动姿势。插值行为要等第一张表真填上才看得到。
