...
| 4 | **槽位 104 个** | ⬜ **全是空 Motion** | 100% 未摆姿势 ⇒ **造型一条都还看不到**；但**轴值现在就能测**（§2.4） |
| 5 | **中间层 profile** | ✅ 三份完全一致（SHA256 相同） | 包内 `Editor/FaceTracking/Profiles/ho-iPhoneVTS.hoface.json` = BREAK_URP rig 副本 = Warudo 沙箱副本；**67 输入 / 127 输出**（90 出口 + 37 轴行） |
| 6 | **轴的修饰符** | ⬜ **37 行一条都没有**（连 `modifiers` 字段都没有） | 曲线也基本是恒等（只是放宽范围防夹断）—— 这是"不稳"的第一嫌疑，见 §3.2 |
...
| --- | --- | --- | ---: | --- | --- | --- |
| `MouthCore` | `Mouth/Form` −1 撇/垂嘴角 · 0 中性 · +1 笑 | `Mouth/Open` 0 闭 · 0.5 半开 · 1 张满 | 9 | 外嘴基础：**唇的轮廓与口孔** | 只写唇/口孔，**不写下颌与下巴** | `MouthCore__Form__Open__A3X<i>Y<j>` |
...
| 超出网格（两侧都测了） | **钳到矩形边界那一行/列**（x=−3 与 x=−1 同值、x=3 与 x=1 同值）—— **不外推、不会超出子节点的值域**；**把整列/整行/单角往里挪之后，照样钳到位移后的那个点** |
| **缺一角**（奇数阵少一个角，我们可能这么稀疏摆） | **内部平滑单调、不压回原点**：中心孩子权重从 0.55 平滑衰减到 0，缺的那块塌成**相邻两个边孩子各半**；但**出界行为不再干净**（(0.5,1.5) 得 102，而满 9 格时是钳制到 (0.5,1) 的 126.75） |
| **退化摆法**（三点表 / 点压在别的点构成的线上） | **不可预测、非单调**：实测到 `(1.5, −0.5, 0)` 这种**负权重**，内部也一样 |

⇒ **"轴超了"最坏只是卡在边界姿势，不会画出畸形造型**。但四件事得记着（详见
...
1. **分辨率白扔**：`Mouth/Open` 实测只到 0.75，而槽位摆到 1 ⇒ 最后 1/4 的插值空间永远用不到
   （把槽位挪到实测范围可以救，且挪完照样安全钳回 —— 见下面的"两条路"）。
2. **树与 Hub 会不一致**：钳制只发生在树内部，Hub / 面板读到的还是超出的原值（"Hub 在涨、姿势不动"）。
3. **"0.4 = 半格"不成立**：插值形状由引擎的距离权重决定，**不能手算**。要精确的死区 / 半格 / 量程，
   只有中间层曲线能给你（那是你自己算的）。
4. **要靠"出界钳到边界"，就必须摆满矩形**（四个角都要）；缺角之后出界落点由引擎几何自己决定。
...
**所以"量程/放大"有两条路**（都能安全钳回，选哪条看你要不要 Hub 里也是那个数）：

| 路 | 怎么做 | 代价 |
| --- | --- | --- |
| **A · 挪槽位**（你的倾向） | 把 `Mouth/Open` 的 Y 刻度从 `0 / .5 / 1` 改成 `0 / .4 / .75`（改生成器的刻度表，重新生成资产） | Hub / 面板 / Warudo 读到的**仍是原始值**（0.75 = 满在那棵树里，别的消费者不知道）；且插值形状本就不可手算，挪完"半格"不在正中 |
| **B · 中间层曲线** | 曲线的横轴是表达式的值、纵轴是写出去的值：`±0.03 → 0`、`0.4 → 0.5`、`0.75 → 1`（`HoFaceCurve.Transfer` 范围外按端点算） | 轴上写出去的就是归一化后的数 ⇒ **Hub / Warudo 一起受益**；但曲线在 profile 里（三份副本要一起更新） |
...
| `Ho/Drive/Mouth/Open` | `(jawOpen - mouthClose) - ((mouthRollUpper + mouthRollLower) * .2) + (mouthFunnel * .2)` | 恒等 −1…2 | — | 0 闭 … 1 张满；**负侧也有值**（抿嘴比闭还"负"）。**实测：张满 0.75、半张 0.4、抿满 −0.16…−0.2、\|x\|<0.03 是"不张也不抿"的死区** | 0 … **0.75**（半张 0.4；抿嘴 −0.2…−0.16） |
...
两个等值关键点若沿用斜率 1 的切线，Unity 会按平滑切线插值、在平台两端**过冲**（表现为"刚要张嘴先动一下"）。
...
2. **按实测结果给 37 行加修饰符 / 修曲线**（§3.2 / §3.3）—— *你定数值，我可以先按同部位量级铺一版草稿*。眼前两条具体的：
   · `Mouth/Open`：曲线加 **±0.03 死区**、`0.4 → 0.5`、`0.75 → 1`（键序 `(−0.2,0)`（或 `(−0.2,−1)` 若要把抿嘴放大）、`(0.03,0)`、`(0.4,0.5)`、`(0.75,1)`，平台段切线给 0）。
...
8. **接线自检**（随时可跑）：面板「参数输出」栏里这 37 行**一个都不该显示"不在控制器里"**；槽位"缺哪些动画"能在
   「详情」栏列出来；`.research/check-controller.ps1` 退出码 0。
