...
| # | 东西 | 状态 | 落在哪 / 判据 |
| --- | --- | --- | --- |
| 1 | **控制器资产** | ✅ 已生成、Unity 已导入 | `BREAK_URP/Assets/Hollow/土豆/FT/PTP_CTR_Face_VTS.controller`；`.research/check-controller.ps1` 报 **0 问题**（42 参数 / 26 树 / **88 槽位** / 1 层 1 状态 / WD 开 / **每棵树都从根可达**）；导入日志全解析、无报错 |
...
| 4 | **槽位 88 个** | 🟡 **已各填一份「空片段」**（2026-09-27） | 片段在 `Assets/Hollow/土豆/FT/Animations/`，**文件名 = 槽位名 = 语义** ⇒ 混合树每格都显示名字而不是 None；空片段不写任何曲线 ⇒ **运行期行为与空 Motion 完全相同**。姿势还一条没摆 ⇒ **造型看不到**，但**轴值现在就能测**（§2.4）。刻度已按实测收过圈（`Open` 的 Y = `0/.4/.75`、`Form` 的 X = `0/+0.75/+1`、**`Forward` = `0/0.12`**、**`Roll` 是开关阈值 `0.02 / 0.12`（√(上×下) 判据 + 死区 + 下颌增益 + 噘嘴门）**；**右上角 `X2Y2` 单独挪到 0.6**） |
...
| 14 | **测值的口（记 5 秒）** | ✅ 2026-09-27 | 「参数输出」栏的 `记 5 秒`：做住一个动作 → 到点自动打一条 Console（每行 `min / avg / max / 波动`，按波动排序；**波动 ≤ 0.01 且 |均值| ≤ 0.01 的行省略**（`HoFaceTrackingWindow.PrintRecord`：`span > 0.01f || |avg| > 0.01f` 才列；所以"偏置大但不动"的行照样列出来 —— 眉那根就是这么抓到的）。**单帧读数不能用来定阈值**（同一动作逐帧差很多）—— 这就是它存在的理由。**测量协议（用户定，之后都照这个走）**：先丢追踪 → 点设备校准、对准 → 连做**三次** 5 秒静止 → 取三次的 `max` 定阈值。普查结果见 §3.5 |
...
│   ├─ MouthForward             **Simple1D 2 格**（下巴前伸：自己一根轴、不跟别人组合）
...
| **`MouthForward`** | `Mouth/Forward` **0 静置 · 0.12 满**（⚠️ 2026-09-27 实测把量程打穿：裸线满量程只有 **0.13**） | —（**1D 表**） | **2** | **下巴前伸**（用户定：自己一根轴、不跟上下左右组合）—— 只写**下颌平移**那几根键；chin / 下唇那些键归 `MouthJaw`，别撞 | 下颌平移 | `MouthForward__Forward__A2X<i>` |
...
* ⭐ **卷唇 = 轴驱动的变体开关**（2026-09-27 用户定，取代了"残差车道"那版）：中间层把
  ...
  | 开（`Roll ≥ 0.12`） | `MouthCoreRoll` | **猫嘴版整嘴**（⭐ 二次元的"猫嘴"：嘴唇内翻压在牙上） |

  实测有两段（只卷嘴 ≈0.18 / 牙齿咬住最强 0.45），用户定**不分开**："不需要区分两段卷嘴" ⇒
  0.12 及以上全停在猫嘴版 = 正确饱和。
  ⚠️ **满档阈值故意低于实测的 0.18**：张嘴时设备会把这根轴压低（见下一条），阈值贴着峰值设会让猫嘴
  在张嘴时只淡入一部分。
  * ⭐ **怎么区分"上下同卷"和"噘嘴"（2026-09-27 四条 5 秒实测定）**：判据就是**两根 roll 线是不是一起动** ——
    轴用 **`√(上×下)`（几何平均）** 而不是平均。四条实测（用户用「记 5 秒」录的均值）：

    | 动作 | `mouthRollUpper` | `mouthRollLower` | 旧口径（平均） | **新口径 `√(上×下)`** | `mouthPucker` |
    | --- | ---: | ---: | ---: | ---: | ---: |
    | 真猫嘴 | 0.355 | 0.576 | 0.466 → 满档 ✓ | **0.452 → 满档 ✓** | 0.025 |
    | **噘嘴** | **≈0** | 0.124 | 0.064 → **44% 猫嘴** ✗ | **0 → 不走猫嘴** ✓✓ | **0.975** |
    | 静息 | ≈0 | 0.016 | 0.008 → 0 ✓ | **0** ✓ | 0.257（基线，别拿来当判据） |
    | M 收音 | 0.021 | 0.039 | 0.030 → 0.010 ✓ | 0.008 ✓ | 0.095 |

    另外第二道独立保险 = **噘嘴门** `1 − clamp((mouthPucker − 0.5)/0.3, 0, 1)`
    —— ⚠️ **门必须开在这台设备的静息基线之上**：`mouthPucker` 静息就有 ≈0.2
    （iPhone 203 帧实测 mean **0.1992**、现场静息 0.17..0.35），开 0.4 只剩 0.05 余量 ⇒ 抬到 0.5
    （噘嘴 0.975 掐死、真猫嘴 0.03 不碰、真卷唇在基线 0.35 时也验证过不受影响）。
    台架上这四条**实测读数**就是断言（181 通过）。
    ⚠️ **M 音**（用户：只有收音闭嘴那一刻带上下卷、那时牙齿是张开的 ⇒ 以后用**下颌相关**的量约束）：
    当前实测 `Roll = 0.008`，够不着开关起点（0.02）⇒ **先不管**，等下巴那套弄清楚再做。
  * **轴 = 上下同卷判据 × 死区 × 下颌增益 × 噘嘴门**（2026-09-27 用户实测 + 用户要求）：
    ① **死区**：`|平均| ≤ 0.02 → 0`、`0.02…0.05` 乘数形式斜坡、0.05 以上**恒等**
       （手机静息时就在这个区间抖；乘数形式保证实测刻度 0.18 / 0.45 原样穿过）；
    ② **下颌增益 `1 + 4·jawOpen²`**：⭐ **用户报"张嘴会把卷唇压到跟噪声分不清"** —— 查过之后确认
       **不是我们的公式在压**（表达式里没有 jawOpen），是**设备本身**在张嘴时把那两根线压低；
       于是我们用这个乘数把它**乘回来**（张大嘴 ×5）。两条设计细节：
       · 增益必须放在**死区之后**：反过来会把噪声一起放大，死区就白设了（说话时张大嘴会乱闪猫嘴）；
       · 增益看 **`jawOpen`**、不看 `Open`：`Open` 的公式里减了 roll（`−0.2×roll`），用它会形成正反馈回路。
    台架上四条断言钉着：`0.01/0.02 ⇒ 0`、`0.09/0.18/0.45 ⇒ 恒等`、斜坡只降不升、
    **闭/半张/张满 ⇒ 0.18 / 0.36 / 0.90**、**被压到 0.05 的张嘴卷唇补回 0.25（≥ 满档）**、
    **噪声在任何下颌开度都仍归 0**。
    ⚠️ **常数 `4` 是待标定的占位**：标定办法见 §2.4 —— 张嘴卷唇时看本行读数，够不到 0.12 就加大它。
  ...
  * ⭐ **键盘/按键要"强制猫嘴"怎么办 —— 直接写这个值**（2026-09-27 用户定）：
    写 `Ho/Drive/Mouth/Roll` ≥ 0.12（"把值加 1 再传过去"也行）就是满档 ⇒
  ...
  * ⚠️ 削掉的是**控制器轴**（`Ho/Drive/Mouth/RollUp`/`RollDown` 两行不再存在）：`mouthRollUpper` / `mouthRollLower`
    ...
    —— **"原始值"那张承诺也没破**：死区与增益只发生在这根**轴**上，G1 的出口行一个字没改。
...
6. **标定卷唇的判据与增益**（§3.1，2026-09-27 改）：用「参数输出」栏的 **`记 5 秒`** 按钮各录一次，
   看 `Ho/Drive/Mouth/Roll` 的 `min/avg/max` —— **① 真猫嘴**（应 ≥ 0.12 = 满档）、**② 噘嘴**（应 = 0.00）、
   **③ 静息**（应 = 0.00）、**④ 张嘴卷唇**（应仍 ≥ 0.12；不够就加大表达式里那个 `4`）。
   两个旋钮都在 `Ho/Drive/Mouth/Roll` 这一行里：`4`（下颌增益）与死区的 `0.02`；
   噘嘴那道门是 `(mouthPucker − 0.5)/0.3` —— ⚠️ **别把阈值压到静息基线以下**：这台设备 `mouthPucker` 静息就有 ≈0.2（实测 mean 0.1992、现场 0.17..0.35）。
   ⚠️ **单帧读数不能用来定阈值**（同一动作逐帧差很多）—— 这就是「记 5 秒」存在的理由。
...
| `Ho/Drive/Mouth/Forward` | `jawForward` | 恒等 0…1 | smooth 0.01 s | 下颌前伸（合同表 `HQJawForward`）。**裸线口径**，喂 `MouthForward` 1D 表 | 静息 **≈0** · 噘嘴单独 **0.05**（串扰，已认）· **噘嘴+前顶 = 0.14 = 满**（满档摆 0.12） |
...
| `Ho/Drive/Mouth/Roll` | `clamp(√(上×下) × clamp((√(上×下) − 0.02)/0.03, 0, 1) × (1 + 4·jawOpen²) × (1 − clamp((mouthPucker − 0.5)/0.3, 0, 1)), 0, 1)` | **恒等**（判据 / 死区 / 增益 / 噘嘴门都写进表达式了） | smooth 0.009 s | **变体开关的输入**（上下两根线的平均，**不分上下唇**）= 合同表 D 的 `HQUpperLipRoll`/`HQLowerLipRoll` **合并成一根轴**；驱动 `MouthCoreRollSwitch`（阈值 **0.02 / 0.12**）在**静息嘴 / 猫嘴版整嘴**之间交叉淡入。**这一行 = 上下同卷判据 × 死区 × 下颌增益 × 噘嘴门**：① **`√(上×下)`（几何平均）** —— ⭐ **上下两根线必须一起动**才算卷唇：两根一起动时 √积 ≈ 平均（旧的 0.18 / 0.45 口径不变），**只动一根时（噘嘴、静息噪声）直接归 0**；② 死区：结果 `≤ 0.02 → 0`、`0.02..0.05` 乘数形式斜坡、之上恒等；③ **`1 + 4·jawOpen²`**：张嘴时设备会把读数压低，乘数把它**乘回来**（用 `jawOpen` 不用 `Open`：`Open` 里减了 roll，用它会形成正反馈回路；增益必须在**死区之后**，否则噪声一起被放大）；④ **`1 − clamp((mouthPucker − 0.5)/0.3, 0, 1)`**：第二道独立保险 —— **门开在 0.5 是因为这台设备的 `mouthPucker` 静息偏置本来就有 ≈0.2**（iPhone 203 帧实测 mean **0.1992**、现场静息 0.17..0.35，见[设备实测](PARAMETER_DEVICE_VERIFICATION.md)），开低了会误掐真卷唇；噘嘴 0.975 照样掐死、真猫嘴 0.03 完全不碰。⭐ **按键强制猫嘴就直接写这个参数 ≥ 0.12**。⚠️ 读数不再是"原始平均"；原始值仍由 G1 出口行无损直通 | **0 … ~1**（闭嘴口径同实测 0.18 / 0.45；张满时 ×5 封顶；静息噪声在任何下颌开度都是 0） |
...
| 轴参数 | profile 表达式 | 曲线 | 修饰符 | 语义 / 值域 | 实测范围 |
| --- | --- | --- | --- | --- | --- |
| `Ho/Drive/Brow/Left/Y` | `2 * ((browOuterUpLeft - browDownLeft * 死区) + ((mouthRight - mouthLeft) / 8))`，死区 = `clamp((browDownLeft − 0.24) / 0.02, 0, 1)` | 恒等 −2…2 | smooth 0.018 s | **0 = 静息**，−1 压眉 · +1 抬眉；VB 那条掺了偏嘴联动（说话时眉会动一点）。⚠️ **死区只掐 `browDown` 那一项** —— 静息时设备就发 0.108…0.2083（§3.5），而**偏嘴那一项故意不掐**（它还活着，说话时眉照动） | 静息 **0**（死区内）· 皱眉/皱鼻 **−1.66**（= −2×0.83，饱和） |
| `Ho/Drive/Brow/Left/InnerUp` | `browInnerUp` | 恒等 0…1 | smooth 0.015 s | 内眉抬起 | 待测 |
| `Ho/Drive/Brow/Right/Y` | `2 * ((browOuterUpRight - browDownRight * 死区) + ((mouthLeft - mouthRight) / 8))`，死区 = `clamp((browDownRight − 0.24) / 0.02, 0, 1)` | 恒等 −2…2 | smooth 0.018 s | 同上（右）；偏嘴那一项左右反号、同样不掐 | 静息 **0** · 皱眉/皱鼻 **−1.66** |
...
| `Brow/*/Y` | 0.018 | = VB `BrowLeftY` / `BrowRightY`（⚠️ 表达式里还挂了 `browDown` 的**死区**，2026-09-27 —— 平滑管抖动、死区管偏置，两件事） |
| `Brow/*/InnerUp` | 0.015 | = VB `BrowInnerUp` |
| `Cheek/*/Squint` · `Nose/*/Sneer` | 0.009 | 肌肉类，与 VB `Brows` 同档（`Nose/Up` 同 0.009，也挂了同形死区） |
...
* 摆姿势时的分工（**Direct 是加法，别撞键**）：`MouthCore` = 唇/口孔；`MouthCorner` = **只有嘴角那几根键**；
  `MouthCoreRoll` = **猫嘴版整嘴**（它是**替换**不是叠加，由开关选中，阈值 0.02 / 0.12）；`MouthWidth` = 嘴宽/偏嘴残差。中间格留空 = **零修正/零残差**（契约 §F 的 fallback）。
* **`MouthCore` 的 X 刻度是三档**（2026-09-27 实测定）：`0 静息 / +0.75 常态笑 / +1 大笑`。
  ...
  **负侧整块不要**（2026-09-27 用户定）：`Form` 的负半边混了三种东西 —— 苦（→ `MouthCorner` 的嘴角）、
  ...
  轴上够得到；要"按键强制猫嘴"就直接写 `Mouth/Roll`（≥0.12，见 §2.2）。眼/眉的副本照留。
  ⚠️ 但**中间层的负值必须留着**（那是正值的抑制量）—— 见 §3.1 那条警告。

---

### 3.5 轴的静息基线（2026-09-27 起：**校准后三次静息**）

⚠️ 这台设备**好几根线的静息不是 0**（是通道偏置，不是我们算出来的）。

**测量协议（用户定，之后每次都照这个做）**：
① 先在设备上**丢一下追踪**（把平滑和一切残留状态清掉）→ ② 点设备上的**校准**、对准镜头
→ ③ 连做**三次** 5 秒静止（面板「记 5 秒」，每次 314…317 帧）→ ④ 定阈值取三次的
**`max`**（最坏情况），看常态取 `avg`。⚠️ **单帧读数一律不作数**（同一动作逐帧差很多）。

三次（01:03:41 / 01:04:20 / 01:04:42；`FaceFound` 恒 1，四个区域门恒 1）：

| 轴 | 三次 avg | 谁在用 | 判定 |
| --- | --- | --- | --- |
| `Brow/*/Y` | **−0.242 / −0.344 / −0.380** | `BrowCoreL/R` X（**两档** −1/+1） | ⚠️⚠️ **根因已定位**：`Y = −2 × browDown`（偏嘴项 ≈0，逐字对上）而 **`browDown*` 静息就有 0.108…0.2083** ⇒ 中性脸落进"压眉"格 **62…71%**。**已修**（`browDown` 项加死区 0.24 / 0.02，见 §3.3） |
| `Nose/Up` | **+0.104 / +0.089 / +0.146** | `NoseUp` 1D（0 / 0.7） | ⚠️ **同一个根因**（`noseSneer*` 静息 0.085…0.1514）⇒ 中性脸白吃鼻上顶格 **13…21%**。**已修**（同形死区 0.18 / 0.02） |
| `Mouth/Pucker` | **−0.090 / −0.083 / −0.087** | `MouthWidth` X（−1/0/+1） | ✅ **校准把它从 −0.29 打到 −0.087** ⇒ 只离静息 8.7%；**做倒V时不必再"归一基线"**，直接按原始 `mouthPucker` 定阈值即可 |
| `Mouth/Form` | **−0.080 / −0.057 / −0.059** | `MouthCore` X（0/0.75/1） | ✅ 负值一律钳到 X0 ⇒ 今天不出错（成因还是 `mouthPucker` 的基线，公式里减了它） |
| `Mouth/Jaw` | **≈0**（`jawOpen − mouthClose` = **+0.0006 / −0.0005 / −0.0002**） | `MouthJaw` Y（0/0.75） | ✅ **天然干净**：两根线**同幅同向**的偏置互相抵消（`mouthClose` 设备侧真的在发，这就是"闭着嘴也能看下颌动"的那根线） |
| `Lid/*/BlinkWide` | **+0.110 / +0.078 / +0.144** | `LidL/R` X（−1/0/+1） | ⚠️ 静息就朝"睁大/眨"侧 **8…14%**（设备把半睁的眼睛报成眨眼；这三次 `EyeOpen` 0.385…0.438）。**眼/眉暂缓 ⇒ 先记着**，真要做眼时同样上死区 |
| `Brow/*/InnerUp` | +0.040 / +0.030 / +0.043 | `BrowCoreL/R` Y（0/1） | 3…4%，先不动（要动就是同一招） |
| `Mouth/JawSide` | ≈0 / ≈0 / **−0.013** | `MouthJaw` X（−1/0/+1） | 1.3%（`jawLeft` 单独静息 0.013、`jawRight` 0）⇒ 忽略 |
| `Cheek/*` · `Shrug` · `Press` · `Funnel` | 0.010…0.034（`MouthShrug` 0.082…0.136） | 无树 | 忽略（耸唇那族还没人用） |
| `Mouth/Open` · `Roll` · `TongueL/R` · `X` · `CornerL/R` · `Forward` | ≈0 | — | ✅ 已在死区内 / 可忽略 |
| `Ho/Drive/Gaze/*` | X 恒 **+1.000** · `Left/Y` −0.38…−1.00 | 无树（Warudo LookAt） | ⚠️ **不是"静息在乱看"**（旧说法已废）：这三根钳死在 +1、`Left/Y` 逐帧**等于** `EyeLeftY`（take C 两边 5 位数完全一致）⇒ 真相是**眼姿态线是"度"（`EyeLeftX` 静息 13.5…16.5°），而这四行的曲线范围写的是 ±1 ⇒ 被端点钳死**。见下面那条 ⚠️ |

* ⭐ **两条根因**：① **`mouthPucker` 静息**（设备级：iPhone 203 帧 mean **0.1992**；现场 0.19…0.26）
  同时造成 `Form` 与 `Pucker` 的负偏置（`mouthDimple*` 也带 ≈0.03 偏置）；
  ② **`browDown*` / `noseSneer*` 静息偏置**（0.108…0.2083 / 0.085…0.1514）——
  这两条**只影响"0 刻度必须是静息"的那几根轴**（`Brow/*/Y`、`Nose/Up`），修法是**死区**，不是偏移。
* ⚠️ **Brow 那条基线不是一个固定常数**（三次 −0.24 / −0.34 / −0.38；更早两轮 −0.22 / −0.36）⇒
  固定偏移一定修不准，**只能做死区**（本次就是这么修的）。
* 📌 **校准会改变基线**：同一台设备校准前 `Mouth/Pucker` −0.29 / `Form` −0.19，校准后 −0.087 / −0.059
  ⇒ **改阈值的记录必须写明"校准前 / 后"**，否则数据不可比。
* ⚠️ **新发现（2026-09-27，三次静息暴露的）：`Ho/Drive/Gaze/{Left,Right}/{X,Y}` 四行的曲线范围写成了 ±1，
  但它们读的是 `EyeLeft_x` / `EyeRight_x` 这种"度"线（±180 的输入行）** ⇒ 任何超过 1° 的眼动都被
  端点钳成 ±1（三次静息实测 `EyeLeftX` 13.5…16.5° ⇒ `Gaze/Left/X` 恒 1.000）。同一份 profile 里
  `EyeLeftX` 那条**出口行**的曲线是 **±90**，所以这是"驱动那四行抄范围时抄错了"，不是设备问题。
  **没改**：① 注视/头部本来就在暂缓名单（交给 Warudo 的 LookAt + IK）；② 用户交代过"中间层里直通的
  玩意儿别删，Warudo 节点要靠这些输入算 LookAt" ⇒ 改范围会**改变 Warudo 收到的数**，得先点头。
  两条候选修法：把这四行曲线放到 **±90**（纯直通、单位留在"度"）或表达式里 **`/30` 归一成 −1…+1**；
  ⚠️ 顺带记一笔：三次静息的水平眼位都是 **+13…+16°**（波动只有 1°）—— 看着显示器而不是看着手机，
  **测眼那族时视线要对着镜头**，否则"静息 0 刻度"无从谈起。
* ⚠️ **`Brow/*/Y` 的第二层问题还在（美术侧，未做）**：`BrowCoreL/R` 的 X 只有 **两档（−1 / +1）**，
  所以就算轴修成静息 = 0，中性脸也是"压眉格 / 抬眉格"各 **50%** 的混合 —— 要真准得给 X **补一个 0 档**
  （三档，`A3X`，每棵树多 2 格；含 `Expr` 副本共 **8 个新格子**）。现在这样已经比 62…71% 好得多，
  且要求两格是**对称**的；等有信号再做（眼/眉本来就暂缓，§5.5）。
* 📌 **`BrowLeftY` / `BrowRightY` / `Brows` 三行出口故意不修**：它们是 VB 同族口径的**照抄行**
  （`Brows` 静息 0.404…0.440 = 0.5 − browDown/2），改了就不再和 VB 对齐；控制器不消费它们。
  修的是**我们自己的驱动轴** `Ho/Drive/Brow/*/Y`。

**串扰登记**（都是实测，摆姿势前要知道）：

| 做这个动作时 | 谁被顺带拉动 | 后果 | 处置（待定） |
| --- | --- | --- | --- |
| **卷唇（猫嘴）** | `mouthFrown*` → 0.52..0.69、`mouthDimple*` → 0.33..0.52 | `Mouth/Pucker` 冲到 **+2 饱和**、`CornerL/R` → **−0.52** ⇒ 会顺带把 `MouthWidth`（展宽侧）与 `MouthCorner`（苦侧）两条残差车道推着走 | ① 作者接受 ② 中间层减污染 ③ **猫嘴激活时把这两条残差车道的 Direct 权重压到 0**（倾向 ③） |
| **噘嘴** | `mouthShrugUpper/Lower` → 0.63 / 0.75（真猫嘴只有 0.23 / 0.39） | 耸唇那族会被带动 | 记录；以后要做耸唇再说 |
| **发 M** | `jawOpen` ≈0.03、`mouthClose` ≈0.03、`Roll` ≈0.008 | 当前够不着开关起点 ⇒ **先不管**（用户：等下巴那套弄清楚再做） | —— |
| **挤眼 / 皱鼻（2026-09-27 实测：挤眼+鼻上抬 5 秒）** | `mouthPucker` 冲到 **0.62**（静息 0.36）、`browDown` **0.83**、`eyeSquint` 0.21..0.29、`cheekSquint` 0.44、`mouthShrug*` 0.41..0.51 | ⚠️ `Mouth/Pucker`（= `2×dimple − pucker`）被拉到 **−0.50**，比**真噘嘴的 −0.47 还低** ⇒ **这根轴不能当"噘嘴检测器"**；真噘嘴要看**原始 `mouthPucker`**（静息 ≤0.43 / 挤眼 ≤0.62 / 真噘嘴 0.975） | 做倒V时用原始 `mouthPucker` 当判据（阈值 ~0.7），必要时再叠"鼻子没在皱"的条件 |
| **噘嘴（2026-09-27 实测）** | 裸 `jawForward` → **0.05**（不动下巴、只噘嘴） | `MouthForward` 1D 表：轴值 0.05 / 满档 0.12 ⇒ 噘嘴单独会顶出 **~42% 的前顶姿势** | ⭐ 用户**已认**（「先原样线性」）。⚠️ **不能照 Roll 用噘嘴门** —— 前顶的**满档本来就要噘嘴参与**（用户：「噘嘴的同时下巴前顶才是最大」），门会把最大值掐掉。要减只能在中间层减掉 `mouthPucker` 那一项（未做；台架有断言钉着「原样不减」） |
| **鼻子上顶的连带** | 用户预期"连带内眼睑上抬 / 眯眼 / 眉毛内下移"**设备自己就给了**：`browDown` 0.83（`Brow/*/Y` 冲到 −1.7 = 饱和在压眉格）、`eyeSquint` 0.21、`cheekSquint` 0.44 | ⇒ **不需要在中间层做联动**（省一层机制） | 已确认 |
## 4. 下一步

按依赖顺序（✅ = 已完成；后面标了谁做）：

1. 🟡 **实机测 31 根轴的灵敏度**（§2.4）—— *进行中*。已回填：`Mouth/Open`（+0.4 半张 / +0.75 张满 / **大笑张嘴 ≈0.6** / **−0.07 不咬唇内卷 / −0.14 咬唇**）、`Mouth/Form`（+0.75 常态笑、+1 大笑、−0.4 卷唇咬唇、−0.5…−0.7 噘嘴/苦脸）、`Mouth/Roll`（两根线一起增减 ⇒ 收成 1D；**只卷嘴 ≈0.18、咬唇最强 0.45** ⇒ 只留 `0 / 0.18` 两档）、`Nose/Up`（挤眼+鼻上抬 **0.70** ⇒ 表里 0.7 档）、`Brow/*/Y`（皱眉/皱鼻 **−1.66** = −2×0.83）、**以及三次校准后静息的整套基线**（§3.5）；其余仍是「待测」。
2. 🟡 **按实测结果给 39 行加修饰符 / 修曲线**（§3.2 / §3.3）—— *你定数值*。已完成 / 待定：
   · ✅ **眉 / 鼻的静息偏置已修**（2026-09-27 用户先校准设备、再连录三次静息 5 秒）：
     ...
     是 0.108 / 0.152…0.205 / 0.163…0.208（**不是常数**，更早两轮 −0.22 / −0.36 也是）⇒ 偏移修不准。
     修的是**驱动轴**；`BrowLeftY`/`BrowRightY`/`Brows` 三行 VB 照抄出口**故意不动**（§3.5）。
     台架 6 条新断言（三次实测静息 → 全 0；皱眉 0.83 → −1.66、鼻上顶 0.70 → 0.70 一点没削），
     已**反向验证**（去掉死区后那 3 条静息断言如期变红：−0.2676 / −0.4092 / −0.4166）。
   · ⬜ **美术侧还差一步（未做）**：`BrowCoreL/R` 的 X 只有两档 ⇒ 轴修成 0 之后中性脸仍是
     "压眉 / 抬眉"各 50%。要真准得补 **0 档**（§3.5 那条）。
   · ✅ **下巴前伸的刻度按实测重落**（2026-09-27 用户定「前顶完全不需要三挡，一档就行」）：
     `MouthForward` 从 **3 格（`0/0.5/1`）砍成 2 格（`0/0.12`）** —— 那个三档是没实测的占位，
     实测裸 `jawForward` 满量程**只有 0.13**（静息 ≈0 / 噘嘴单独 0.05 / **噘嘴+前顶 0.14 = 满**）。
     两个生成器 + `check-controller` 一起改，`:controller` 重新生成（槽位 **89 → 88**，
     旧 `MouthForward__Forward__A3X0/1/2` 三个片段按孤儿规则删掉、新建 `A2X0/1`）。
     口径 = **原样线性**，噘嘴那 0.05 的串扰用户已认（台架断言钉着「一点不减」）。
   ...
   · ✅ **卷唇改成"轴驱动的变体开关" + 下颌增益**（2026-09-27 用户定）：`Mouth/Roll`（两根线平均、不分上下唇、**死区 × `1 + 4·jawOpen²`**）驱动 `MouthCoreRollSwitch`（阈值 0.02 / 0.12），在**两张作者画过的整嘴表**（`MouthCore` 静息嘴 / `MouthCoreRoll` 猫嘴版）之间交叉淡入。**用户报"张嘴把卷唇压到跟噪声分不清"后加的增益**：查实不是我们的公式（表达式里没有 jawOpen），是设备在张嘴时压低那两根线 ⇒ 乘数乘回来。用户的两条判断：① 「不需要区分两段卷嘴」⇒ 0.18 及以上全停在猫嘴版；② 反对"几条残差相加"——混态得有人负责 ⇒ 所以是**分叉**而不是再加一条残差车道。代价说好了：作者要为用到的格**画整张嘴**，工具不替他拷贝。台架 3 条断言钉死死区与恒等段。
   · ✅ **嘴的按键表情版已删**（2026-09-27 用户定）：`MouthCoreExpr` + `MouthCoreSwitch` 整个去掉（夸张的笑嘴 = `Form` 更大，轴上够得到）；要"按键强制猫嘴"就直接写 `Mouth/Roll` ≥0.12。眼/眉的副本保留。**嘴的第二张表现在是"变体"**（`MouthCoreRoll`，由轴选）而不是"副本"（由表情门选）—— 机制同、语义不同。
...
## 5. 目标姿势清单与缺口（2026-09-27 用户定的最终目标）

### 5.1 一条规则：**要不要跟「笑 × 张」耦合**，决定它做成什么

| 形状 | 跟 `Form`（笑）/ `Open`（张）的关系 | 该用什么 | 代价 |
| --- | --- | --- | --- |
...
| **下巴前伸** | ✅ **已落地** | `MouthForward`（**1D，2 格：静置 / 满**） | 自己一根轴、不跟别人组合（用户定）；⚠️ **满档要「噘嘴 + 前顶」一起做**（2026-09-27 实测） |
...
现在 `Mouth/Form` 的负侧**同时被两件事驱动**：`mouthFrown*`（真·苦）与 `mouthPucker`（噘，是嘴形不是情绪）。
而且**卷唇也会把 `mouthFrown*` 点到 0.52..0.69**（实测）⇒ 直接补负侧会得到两个假象：
"噘嘴 = 苦"、"猫嘴 = 苦"。所以要三件事一起改：

1. **从 `Form` 的公式里去掉 `mouthPucker`**（噘嘴已经有自己的轴 `Mouth/Pucker`，而且用户明确要"噘 → 倒V"而不是苦）；
2. **负侧（frown）去卷唇污染**：`frownClean = clamp(frown − k × 卷唇判据, 0, 1)`（k **待实测**）；
3. **加一档实测的苦刻度**（`Form` 的 X 负侧，例如 −0.5，等「苦嘴」5 秒定），
   这样 `MouthCore` 变成"**苦 ↔ 静息 ↔ 常态笑 ↔ 大笑** × 张多大"，苦嘴**自动**与笑互斥、与张嘴耦合。

⚠️ 需要一次「**苦嘴**」的 5 秒（顺便看 `mouthPucker`/`mouthRoll*` 在苦脸时是多少），才能定 k 与那一档。
### 5.4 下巴树：上下 × 左右（**2026-09-27 用户定**）

**一张树管完"下巴上下左右"** —— `MouthJaw` 的 Y 从 `Forward`（前伸，用户没要）换成**左右**，X（上下）**把 `mouthClose` 并进来**：

| 轴 | 公式 | 语义 |
| --- | --- | --- |
| `Ho/Drive/Mouth/Jaw`（上下，**双极**） | `jawOpen − mouthClose` | **0 = 静息 · + = 张开 · − = 咬合**（下颌向上顶、牙齿合上） |
| `Ho/Drive/Mouth/JawSide`（左右，**双极**） | `jawRight − jawLeft` | **0 = 中 · − = 偏左 · + = 偏右** |
| `MouthJaw` | 上下 × 左右 | 下巴的整个运动空间 |

⭐ **咀嚼不需要第二层**（用户判断，我先前说"要第二层"是错的）：**咀嚼 = 这根上下轴上的动作** ——
闭着嘴唇时下颌上下动，`mouthClose` / `jawOpen` 会振荡 ⇒ 树在"静息 / 咬合"两格之间来回插值
（同时 `MouthCore` 的 Y 也在动）。作者只要把那两格画好，动作就出来了。

⚠️ **`mouthClose` 只有 ARKit/iPhone 有**：安卓那份直通参考**根本没有这一行**（65 输入 vs iPhone 67）
⇒ 安卓侧这根轴只剩正侧（"咬合 / 咀嚼"检不出来）。这条要写进给安卓用的那份配置说明。

**`Mouth/Forward`（前伸）自己一棵 1D 小树**（2026-09-27 用户定：「下巴前伸就是一个单独的轴，他不用跟别人组合」）——
不进 `MouthJaw`、不跟上下左右组合：`MouthForward` = 1D，和 `MouthJaw` 靠加法叠加
（分工：前伸表只写**下颌平移**那几根键，chin/下唇那些键归 `MouthJaw`，别撞）。
⭐ 这也是**那套 1D 片段表机制**（`SlotKids1D` / `Simple1DTables`）的第一个正式用户 —— 之前为卷唇留的扩展口终于用上了。

⚠️ **刻度从三档砍成两档（2026-09-27 用户实测 + 用户定「前顶完全不需要三挡，一档就行」「静置跟满」）**：
原来那套 `0 / 0.5 / 1` 是**没实测就占位的**，一测就把量程打穿了 —— 裸 `jawForward` 的满量程
**只有 0.13**（两次读数 0.13 / 0.14）：

| 状态 | 裸 `jawForward` | 落在这棵树上 |
| --- | ---: | --- |
| 静息 | ≈ 0（三次校准后静息里它**不在「动过」名单内**） | 0 |
| **噘嘴（不动下巴）** | **0.05** | 轴值 0.05 ⇒ **~42% 的前顶姿势**（串扰，见下表） |
| **噘嘴 + 前顶** | **0.14** | 满 |

* ⭐ **满档要「噘嘴 + 前顶」一起做**（用户原话：「噘嘴的同时下巴前顶才是最大」）⇒
  ⚠️ **Roll 那种「噘嘴门」在这根轴上不能用** —— 门会把最大值一起掐掉。两档表的刻度 = `0 / 0.12`。
* 满档摆 **0.12** 而不是踩在 0.13/0.14 上：照 `Roll` 那个先例（满档**故意低于**实测峰值）留余量，
  弱一天也能真的到 100%。这是**一个数**，想贴峰值改一处即可。
* ⚠️ **噘嘴那 0.05 的串扰用户已认**（口径 = 「先原样线性」）：噘嘴单独会顶出 ~42% 的前顶姿势。
  要减就是中间层再减掉 `mouthPucker` 贡献的那一项（**未做**，台架有一条断言把「原样不减」钉着，
  改的时候会响）。

**格子数（2026-09-27 用户定：六个）**：用户说"下颌移动时没有上的，只用**六个**状态"，采用
**上下 2 档 × 左右 3 档 = 6 格**（咀嚼的"上"就是"静息/闭"那一档）；备选：
**上下 3 档 × 左右 3 档 = 9 格**（"咬合"单占一档）、或**中间一列 3 档 + 静息行的左右 2 格 = 5 格**。
⚠️ 定下来之前不动生成器。
...
* **眼 / 眉这一族暂时不动**，树里就让它们**ARKit 直通**（现状就是：axes 由线直接/等价映射而来、
  `LidL/R` + `BrowCoreL/R` 各一棵，`Expr` 副本等门来源）；
* **注视与头部朝向不做树** —— **Warudo 有单独的 LookAt 节点（带 IK）**管这件事；我们的注视/姿态
  出口行照旧发布（谁要用谁取）；
* **颊不要、鼻只要一个状态**（"鼻子上顶"，见上表）；
