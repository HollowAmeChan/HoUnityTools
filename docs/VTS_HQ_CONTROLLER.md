...
| --- | --- | --- | ---: | --- | --- | --- |
| `MouthCore` | `Mouth/Form` −1 苦/垂嘴角 · 0 静息 · **+0.75 常态笑** · **+1 大笑**（**按实测定**：常态笑落在 0.75，大笑才到 1 ⇒ 两个真实状态各要一格） | `Mouth/Open` **−0.14 咬唇内卷 · 0 闭 · 0.4 半开 · 0.75 张满**（**按实测定**：负侧是不咬唇内卷 −0.07 / 咬唇内卷 −0.14；正侧张满只到 0.75、半张 0.4） | 12 | 外嘴基础：**唇的轮廓与口孔**（稀疏：可达区是斜带，两个对角死角已挖掉） | 只写唇/口孔，**不写下颌与下巴**；嘴角的夸张与不对称归 `MouthCorner` | `MouthCore__Form__Open__A4X<i>Y<j>` |
...
* **`MouthCore` 的 X 刻度是四档**（2026-09-27 实测定）：`−1 苦 / 0 静息 / +0.75 常态笑 / +1 大笑`。
  正侧不是"0 → 1"两档，因为**常态笑（0.75）与大笑（1）都是真实状态，各要一个采样点** ——
  只有两档的话，常态笑会变成"75% 的大笑"，而不是它自己的姿势。负侧仍停在 −1（曲线在那里钳住，
