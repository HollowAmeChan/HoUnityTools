...
| 14 | **测值的口（记 5 秒）** | ✅ 2026-09-27 | 「参数输出」栏的 `记 5 秒`：做住一个动作 → 到点自动打一条 Console（每行 `min / avg / max / 波动`，按波动排序）。**单帧读数不能用来定阈值**（同一动作逐帧差很多）—— 这就是它存在的理由。普查结果见 §3.5 |
...
### 3.5 轴的静息基线（2026-09-27 普查）

⚠️ 这台设备**好几根线的静息不是 0**（是通道偏置，不是我们算出来的）。普查数据：用户静息 5 秒（「记 5 秒」录的）
+ [设备实测](PARAMETER_DEVICE_VERIFICATION.md)（iPhone 203 帧 / 安卓 223 帧）。

| 轴 | 静息 avg | 谁在用 | 判定 |
| --- | ---: | --- | --- |
| `Mouth/Form` | **−0.13 … −0.19** | `MouthCore` X | 负值钳到 X0 ⇒ 今天不出错；**成因是 `mouthPucker` 的基线**（公式里减了它） |
| `Mouth/Pucker` | **−0.29**（−0.35…−0.23） | `MouthWidth` X | ⚠️ 静息就 29% 朝"噘/倒V"那侧 ⇒ **做倒V前必须归一** |
| `Brow/*/Y` | **−0.22**（另一轮 −0.36） | `BrowCoreL/R` X（**两档** −1/+1） | ⚠️⚠️ 静息落在 −0.22 ⇒ 中性脸**已经 61% 是"压眉"那格**（两档表最怕静态偏置）；待修 |
| `Nose/*/Sneer` | 0.065 | `NoseSneer` | 6.5%，小但非零 ⇒ 记着 |
| `Lid/*/BlinkWide` · `Squint` | +0.056 · 0.035 | `LidL/R` | ⚠️ **数据不干净**（那次静息 `EyeOpen` 只有 0.44 = 半闭眼） |
| `Gaze/*` | −0.98…+1.00 | `GazeL/R` | ✗ **数据不可用**（那次在到处看） |
| `Mouth/Open` · `Roll` · `TongueL/R` · `X` · `CornerL/R` · `Jaw` · 颊 | ≈0 … 0.02 | — | ✓ 已在死区内 / 可忽略 |

* ⭐ **一个根因**：`mouthPucker` 静息 **≈0.26**（设备级：iPhone 203 帧实测 mean **0.1992**、range 0.12；
  现场 0.17..0.35）同时造成 **`Form` 的 −0.13** 与 **`Pucker` 的 −0.18** ⇒ **修这一根线能同时治好两个**。
  同族：`mouthDimple*` 也有 ≈0.06 偏置。
* ✅ **2026-09-27 干净静息已录**（睁眼、嘴闭、盯镜头）：`Mouth/Pucker` **−0.29**、`Mouth/Form` **−0.19**、
  `Brow/*/Y` **−0.22**；其余都很小（`Lid/*/BlinkWide` +0.07、`Squint` 0.022、`Cheek/*` 0.024/0.012、
  `Nose/Up` 0.073、`CornerL/R` −0.025/−0.020、`X` ≈0、`Jaw` −0.012、`JawSide` −0.03、`Forward` 0.031、`Roll` 0）。
  ⚠️ 两轮静息的 `Brow`（−0.36 / −0.22）说明那条**基线不是固定常数**（他们的中性脸 + 设备偏置一起动）⇒
  给 `Brow/*/Y` 做固定偏移不如做**死区**。
* ⚠️ **单帧读数不能用来定阈值**（同一动作逐帧差很多）⇒ 一律用「记 5 秒」的 `min/avg/max`。
