...
| **下巴左右 + 上下** | ⚠️ 要改（**线都有**） | **一张树管完**：`MouthJaw` = 上下 × 左右（见 §5.4） | 加 1 根轴（`Mouth/JawSide`）+ 上下轴加进 `mouthClose` + 换掉 Y（原 `Forward`） |
...
⚠️ **`Mouth/Forward`（前伸）因此失去 Y 位** —— 变成"发布了但没有树用"的轴（和 `Funnel` / `Press` 一样，
留着以后要用再接）。

**格子数（待定，候选）**：用户说"下颌移动时没有上的，只用**六个**状态" ⇒ 最可能是
