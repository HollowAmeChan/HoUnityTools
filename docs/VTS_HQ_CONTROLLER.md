...
* ⭐ **卷唇 = 轴驱动的变体开关**（2026-09-27 用户定，取代了"残差车道"那版）：中间层把
  ...
  在张嘴时只淡入一部分。
  * **轴 = 死区 × 下颌增益**（2026-09-27 用户实测 + 用户要求）：
...
6. **标定卷唇的下颌增益**（§3.1，2026-09-27 新增）：做三个动作看 `Ho/Drive/Mouth/Roll` 的读数 ——
   **① 闭嘴卷唇**（应 ≈0.18）、**② 张大嘴卷唇**（**应 ≥ 0.12**；这是开关的满档线，够了就是整张猫嘴）、
   **③ 静息张嘴**（应 = 0.00）。②不够就加大 `Ho/Drive/Mouth/Roll` 表达式里那个 `4`（`1 + 4·jawOpen²`）；
   ③不是 0 就把死区的 `0.02` 抬一点。两个数都只动这一行。
...
| `Ho/Drive/Mouth/Roll` | `clamp(平均 × clamp((\|平均\| − 0.02)/0.03, 0, 1) × (1 + 4·jawOpen²), 0, 1)`（平均 = `(mouthRollUpper + mouthRollLower)/2`） | **恒等**（死区与增益都写进表达式了） | smooth 0.009 s | **变体开关的输入**（上下两根线的平均，**不分上下唇**）= 合同表 D 的 `HQUpperLipRoll`/`HQLowerLipRoll` **合并成一根轴**；驱动 `MouthCoreRollSwitch`（阈值 **0.02 / 0.12**）在**静息嘴 / 猫嘴版整嘴**之间交叉淡入。**这一行 = 死区 × 下颌增益**：① `\|平均\| ≤ 0.02 → 0`、`0.02..0.05` 乘数形式斜坡、0.05 以上**恒等**（实测刻度 0.18 / 0.45 不被挪位）；② **`1 + 4·jawOpen²`**：张嘴时设备会把这个读数压低，乘数把它**乘回来**（用 `jawOpen` 不用 `Open`：`Open` 里减了 roll，用它会形成正反馈回路；增益必须在**死区之后**，否则噪声一起被放大）。⭐ **按键强制猫嘴就直接写这个参数 ≥ 0.12**。⚠️ 读数不再是"原始平均"；原始值仍由 G1 出口行无损直通 | **0 … ~1**（闭嘴口径同实测 0.18 / 0.45；张满时 ×5 封顶；静息噪声在任何下颌开度都是 0） |
