...
| # | 东西 | 状态 | 落在哪 / 判据 |
| --- | --- | --- | --- |
| 1 | **控制器资产** | ✅ 已生成、Unity 已导入 | `BREAK_URP/Assets/Hollow/土豆/FT/PTP_CTR_Face_VTS.controller`；`.research/check-controller.ps1` 报 **0 问题**（43 参数 / 29 树 / 1 层 1 状态 / WD 开 / **每棵树都从根可达**）；导入日志全解析、无报错 |
| 2 | **参数表 43 个** | ✅ | 5 区域门 = `1`、`Ho/Drive/W/One` = `1`、2 个表情门 = `0`、31 根轴 = `0`、4 条切片权重 = `0` |
| 3 | **树形 29 棵** | ✅ | 根 `Ho/00 Drive Tree`（Direct）→ 5 个区域 Direct → **15 张表（14 张 2D + 1 张 1D）** + 4 张副本 + 4 棵 1D 开关树。⚠️ 嘴**没有副本也没有开关**（2026-09-27 删掉 `MouthCoreExpr`/`MouthCoreSwitch`：夸张的笑嘴 = `Form` 更大，轴上够得到） |
| 4 | **槽位 105 个** | 🟡 **已各填一份「空片段」**（2026-09-27） | 片段在 `Assets/Hollow/土豆/FT/Animations/`，**文件名 = 槽位名 = 语义** ⇒ 混合树每格都显示名字而不是 None；空片段不写任何曲线 ⇒ **运行期行为与空 Motion 完全相同**。姿势还一条没摆 ⇒ **造型看不到**，但**轴值现在就能测**（§2.4）。刻度已按实测收过圈（`Open` 的 Y = `0/.4/.75`、`Form` 的 X = `0/+0.75/+1`、**`Roll` 的 = `0/.18`（带 ±0.02 死区）**，负侧交给别的表；**右上角 `X2Y2` 单独挪到 0.6**） |
...
### 2.1 结构（29 棵 = 1 根 + 5 区域 + 15 张表 + 4 副本 + 4 个 1D 开关）
...
│   ├─ MouthLipRoll             **Simple1D 2 格**（卷唇：不卷 / 只卷嘴=猫嘴 —— 一个开关）
...
⚠️ **`MouthCore` 直接挂在区域下**（2026-09-27）：删掉 `MouthCoreSwitch` 之后它就不是开关的孩子了。
**这类"删了开关忘了重新挂载"会让整张表变成孤儿树**（树还在、槽位名也对，但状态走不到它 ⇒ 那一整块姿势丢失）——
当时是**片段探针**先撞出来的（它从状态数到的片段少了 8 个），现在 `.research/check-controller.ps1` 也会从根走一遍可达性并报错。
...
| **`MouthLipRoll`** | `Mouth/Roll`（轴带 **±0.02 死区**）**0 不卷 · 0.18 只卷嘴（= 猫嘴）** | —（**1D 表，没有 Y 轴**） | **2** | **卷唇开关**：关 = 静息嘴、开 = **猫嘴**（嘴唇内翻压在牙上）；实测的两段（只卷嘴 0.18 / 咬唇 0.45）**并成一档** —— 0.18 及以上都停在猫嘴 | 只写**卷唇那几根键**（张嘴时这棵树照样生效 ⇒ 必须只动卷唇的键，别碰口孔） | `MouthLipRoll__Roll__A2X<i>` |
...
* ⚠️ **1D 表的阈值 = 刻度值本身，必须显式写死**（`m_UseAutomaticThresholds: 0`）：自动模式会**忽略**存下来的阈值、
  在 `[m_MinThreshold, m_MaxThreshold] = [0,1]` 上把两档平摊成 0 与 1。`Roll` 的 `0 / 0.18` 不是等距的，
  靠自动模式会**静默错位** —— 生成器与 Unity 端两边都写死。
* ⚠️ **`MouthLipRoll` 是 1D，而且只剩两档**（2026-09-27 用户实测定）：原来拿 `mouthRollUpper` / `mouthRollLower`
  两张 2D 轴，实测**两根线一起增减**（"横向纵向都在表达同一件事"）⇒ 第二维是死的；中间层把两根线平均成一根
  `Mouth/Roll`，表跟着收成 1D。
  **语义：一根轴 = "牙齿咬住 + 嘴唇内卷的程度"**（用户原话："根本就不需要跟 ARKit 那样分上下"）。
  实测有两段，但用户定**不分开**：「不需要区分两段卷嘴，压成一个开关都行」⇒ 表就是**一个开关**：

  | 档 | 轴值（实测） | 是什么 |
  | --- | --- | --- |
  | `X0` | **0** | 不卷（静息嘴） |
  | `X1` | **≈ 0.18** | **只卷嘴** = ⭐ **二次元的"猫嘴"**（嘴唇内翻、牙齿不动） |

  **咬唇（实测最强 0.45）并入 `X1`**：轴上高于 0.18 一律停在猫嘴那一档 = 正确饱和。
  * **轴上有 ±0.02 死区**（2026-09-27 用户实测）：`|值| ≤ 0.02 → 0`、`0.02…0.05` 斜坡、之外**恒等**
    —— 和 `Mouth/Open` **同一条形状**（手机静息时就在这个区间抖）。实测刻度 `0.18` 原样过曲线，
    台架上有三条断言钉着（`0.01/0.02 ⇒ 0`、`0.09/0.18/0.45 ⇒ 恒等`、斜坡只降不升）。
  * ⭐ **键盘/按键要"强制猫嘴"怎么办 —— 直接写这个值**（2026-09-27 用户定）：
    **按键就写 `Ho/Drive/Mouth/Roll`**（写 `0.18` 或更大、甚至"把值加 1 再传过去"都行），
    因为 ≥0.18 就是满档 ⇒ **不需要第二张表、也不需要表情门**。（这也正是删掉嘴的按键表情版的底气：
    嘴的"夸张"永远可以用同一套轴表达，只有眼/眉那种轴上到不了的"性质"才需要副本表。）
  * ⭐ **为什么它自己一张表（用户给的机制）**：**卷唇在张嘴时也能被识别到** —— 张嘴时读数会**逐渐降低**，
    但仍然**能到 `X1` 那一档** ⇒ 这根轴必须在"闭嘴"和"张嘴"两种状态下都活着，
    所以它不能并进 `MouthCore`（那棵树只答"笑 × 张多大"）。
    **闭嘴卷唇 = 最准确的猫嘴**（`X1` 那格就是照闭嘴状态摆的）。
    ⚠️ 推论（摆姿势时的纪律）：`Direct` 是加法 ⇒ 张嘴卷唇时 `MouthCore`（口孔）与 `MouthLipRoll`（卷唇）
    **会同时生效并相加**，所以这两棵表的键集合必须守 §2.2 的分工，**不能互相写对方的键**。
...
* **不能测**：造型好不好看 —— 105 格现在都是**空片段**（没有曲线的命名 clip），插值出来的还是启动姿势。插值行为要等第一张表真填上才看得到。
...
| `Ho/Drive/Mouth/Roll` | `(mouthRollUpper + mouthRollLower) / 2` | **±0.02 死区 + 0.02..0.05 斜坡，之外恒等**（和 `Mouth/Open` 同形状） | smooth 0.009 s | **卷唇开关的输入**（上下两根线的平均，**不分上下唇**）= 合同表 D 的 `HQUpperLipRoll`/`HQLowerLipRoll` **合并成一根轴**；喂 `MouthLipRoll`（**1D 表，两档**）。实测：**0 不卷 / 0.18 只卷嘴（= 二次元猫嘴）/ 0.45 咬唇最强** ⇒ 表只摆 `0 / 0.18` 两档，**0.18 及以上都停在猫嘴**。⭐ **张嘴时这根轴照样有值**（读数随张嘴降低但仍能到 0.18）⇒ 它就是为此独立成表的；闭嘴卷唇 = 最准的猫嘴。⭐ **按键要强制猫嘴就直接写这个参数**（写 ≥0.18 即可，不需要第二张表/表情门）。⚠️ 两根线本身没被删：`mouthRollUpper`/`mouthRollLower` 仍是 profile 的输入行与出口行，只是**控制器轴**不再各占一维 | **0 … 0.45**（0.18 = 只卷嘴起点，往上到 0.45 = 咬唇最强；静息噪声被死区抹平） |
...
* **`MouthCore` 的 X 刻度是三档**（2026-09-27 实测定）：`0 静息 / +0.75 常态笑 / +1 大笑`。
  ...
  **负侧整块不要**（2026-09-27 用户定）：`Form` 的负半边混了三种东西 —— 苦（→ `MouthCorner` 的嘴角）、
  ...
  `MouthCore` 8 格 + `MouthLipRoll` 2 格 ⇒ **嘴这块 10 格**；全控制器 **105 格**。
...
2. 🟡 **按实测结果给 40 行加修饰符 / 修曲线**（§3.2 / §3.3）—— *你定数值*。已完成 / 待定：
   ...
   · ✅ **`MouthLipRoll` 从 2D 收成 1D，刻度按实测 `0 / 0.18`，并加了 ±0.02 死区**（2026-09-27 用户实测定）：上下两根卷唇线一起增减 ⇒ 中间层平均成一根 `Mouth/Roll`；语义是"牙齿咬 + 内卷嘴的程度"（**不分上下唇**），**只剩两档**（用户定"不需要区分两段卷嘴，压成一个开关都行"）：不卷 / **只卷嘴 = 二次元猫嘴**（实测咬唇最强 0.45 并进这一档）。死区与 `Mouth/Open` 同形状（手机静息噪声）。**张嘴时这根轴照样有值**（读数降低但能到 0.18）⇒ 它就是为此独立成表；闭嘴卷唇是最准的猫嘴。文本生成器与 Unity 生成器两边同步改完，台架新增 3 条断言钉死死区与"刻度原样过曲线"。
   · ✅ **嘴的按键表情版已删**（2026-09-27 用户定）：`MouthCoreExpr` + `MouthCoreSwitch` 整个去掉（夸张的笑嘴 = `Form` 更大，轴上够得到）；要"按键强制猫嘴"就直接写 `Mouth/Roll` ≥0.18。眼/眉的副本保留。树 31 → **29**、槽位 114 → **105**。
...
4. ✅ **1D 树 + 子树嵌套已实测**（2026-09-27，见 §2.3.1 与[能力边界](BLEND_TREE_LIMITS.md) §10）：
   ...
   **四层嵌套（Direct → Direct → 1D → 2D）照旧求值**。兜底形状（Direct 兄弟 + 中间层补权重）**不需要**了。
   ...
   43 参数 / 29 树，`MouthCore` 的 8 格坐标与 `MouthLipRoll` 的两档阈值，与文本生成器**逐位一致**。）
