...
- [**动态参数（语义输出）**](FACE_TRACKING_DYNAMIC_PARAMETERS.md)：控制器那条**很薄的第二输出**怎么走 —— 值在 `HoFaceSemanticHub`（纯存值，槽由写的人运行期按名字开）、接口是 `HoFaceSemanticConnector`（**没有表**）、**写的人是中间层**（Unity 会话 / Warudo 的「HoFace写动态参数」节点，名字 = 配置里输出行的 `parameter`）、`P/` 前缀只在生成期用。含"为什么删掉那个 ScriptableObject 资产"（实测：它那半边**从来没有过消费者**）、"为什么把动画曲线换成状态机行为、**当天又把状态机行为也删了**"（§5.1 把走错的两条路留了档），以及 ⚠️ **还没验**的那条：Warudo 侧的节点连线与面板。
