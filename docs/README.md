...
- [面捕控制器结构：Jerry 模板 vs 我们的做法](FACE_TRACKING_CONTROLLER_STRUCTURE.md)：参考实现的三层结构、一棵 Direct 树管整张脸、**控制器是作品而初始化只负责搬运**（整份复制 + 重绑驱动对象 + 保留 GUID），以及「Direct 树 + 写默认值关闭会发散」的判别性实验。
...
- [面捕混合树入门与 Jerry ARKit 模板使用](archive/FACE_TRACKING_TEMPLATE_GUIDE.md)：讲 Jerry 现成模板的用法；我们改成搬运模型后，作业流程部分不再适用，但"模板里长什么样"仍是对照材料。
