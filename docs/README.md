...
- [**HO 参数规范**](PARAMETER_HO.md)：**我们自己选什么** —— 输入契约（iPhone 实测 67 线，52 个 ARKit 形态量**全部在动**）、值域约定（官方只规定了两个参数的范围；**0.5 中立位是 VB 私有的**）、目标词表（官方 VTS **20** + VB 自造 **16** + 协议层信号 `FaceFound`，共 **37**；逐个给含义/值域/源/公式来源）、公式全表（机械提取，不手抄）、以及**明确算不出来的 9 行 / 11 个及原因**。跟标准表的分工：那份记外部怎么定，这份记我们选什么。
