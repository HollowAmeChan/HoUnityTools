...
- [**VTS 原生语义控制器：2D 混合树规划（草案）**](VTS_HQ_CONTROLLER.md)：**这一版怎么落地** —— 哪些语义进哪张 2D 表（三档：VB 已有可直接合 / 加一根 HQ 行 / 先一维）、区域门 × **按键表情平行副本**门（嵌套已确认可接受）、键集合分工规则、tranche 1 的 8 族与槽位命名、土豆那份配置的改造清单。含一条实测事实：手机协议的 `Hotkey` **恒为 −1**（两台设备都是），所以按键表情的来源得另接。
